...
| P4 causal development | `P4V2R2_ZERO_CALL_BOOTSTRAP_REPAIRED` | The first autonomous launch reached machine GREEN but invoked `/usr/local/bin/python3`, which lacks MLX, instead of the already-qualified project runtime. It failed before model load with 0/60 calls, zero candidates, zero runtime receipts, and 0/10 completed tasks, so it is an instrument bootstrap defect with no scientific inference and no task consumption. Exact failure evidence is preserved at commit `209f2f2e`; the attempt-2 controller binds the MLX 0.32.0 / MLX-LM 0.31.3 project runtime, probes that executable, excludes only Darwin exit/dead process states, requires a positive Metal arithmetic-and-synchronize canary, writes a separate report namespace, and retains the uncapped completion contract. All host and runtime preflight gates are GREEN for autonomous relaunch |
