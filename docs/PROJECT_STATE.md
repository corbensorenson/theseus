...
| Semantic-IR independent adequacy | `SOURCE_REVISION_POLICY_PREREGISTERED_ZERO_EVALUATOR_OR_MODEL_CALLS` | The powered 18-repository design and six strata are unchanged. Two source-only task-4 failures are preserved: an unchanged selected path with a corrected 36-to-34 counter overcount, then proof that the rebased PR's `merge_commit_sha` identifies a docs-only final commit rather than the PR-head source repair. A prospective policy now freezes public PR `base.sha` to `head.sha` for task bytes and retains merge/first-parent identities as lineage only. Metadata preflight is GREEN with zero calls; evaluator, model, Luna, training, D1, and D2 counters remain zero |
