...
| P4→D1 successor handoff | `AUTONOMOUS_SOURCE_AND_ARCHIVE_STAGES_WAITING_FOR_EXACT_P4V2R2_SURVIVOR` | D1 binds the exact P4-v2r2 disposition and TMax snapshot rather than historical P4S/Qwen owners. Its source successor waits indefinitely without a user gate, opens public metadata only after the exact survivor status, reacquires only after a new complete UTC interval, and freezes the first eligible 44-repository cohort exactly once. A second exclusive waiter then materializes exact parent/target archives, preserves upstream bytes, rejects unsafe members, normalizes deterministically, and checks changed-member and license custody without execution. Evaluator/task-prompt sealing, the blind candidate/control campaign, independent integrity recomputation, and terminal disposition remain unimplemented and must not be described as D1-ready |
