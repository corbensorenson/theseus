...
| Semantic-IR independent adequacy | `SOURCE_MATERIALIZATION_GREEN_EVALUATOR_QUALIFICATION_OPEN` | The powered 18-repository design and six strata are unchanged, and both source-only task-4 failures remain preserved. The repaired public PR `base.sha` to `head.sha` policy materialized 18/18 pairs into 36 deterministic archives through 78 calls. An independent network-free audit verified 36 archive receipts, 76 member receipts, normalization/path safety, exact directory membership, and selected-source differences for every task. Evaluator, model, Luna, training, D1, and D2 counters remain zero; parent-negative/target-positive evaluator qualification is active |
