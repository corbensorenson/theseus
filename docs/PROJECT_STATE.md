...
| Semantic-IR independent adequacy | `SOURCE_PATH_AMENDMENT_PREREGISTERED_ZERO_EVALUATOR_OR_MODEL_CALLS` | The powered 18-repository design and six strata are unchanged. Initial materialization failed closed on task 4's unchanged selected path and exposed a 36-versus-34 archive counter overcount; both the RED receipt and audit are preserved. A versioned single-path amendment selects the actual `cli.py` predicate repair in the same PR and stratum and is preflight-GREEN before renewed metadata acquisition. Evaluator, model, Luna, training, D1, and D2 counters remain zero |
