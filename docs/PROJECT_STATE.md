...
| Active VCM claim | `VCM_V3_K2_03_RISK_EXECUTOR_V3_PROSPECTIVELY_SEALED` | V2 qualified Bun and exposed the Yarn Node-engine gap. Official project-local Node 22.22.2 is now checksum-verified and binary-bound; V3 is sealed to resume without Bun reacquisition, then run Yarn, TypeScript, and Rust |
...
`execute_v3_idempotent_bun_replay_then_yarn_typescript_rust_and_role_separately_audit`.
...
binary hashes bound. V3 must verify and reuse the existing Bun store, resume
the remaining risk classes, and then undergo role-separated audit.
