...
| Active VCM claim | `VCM_V3_K3_NONSCORING_PHYSICAL_HOST_CANARIES_READY` | K2 is GREEN for one contiguous six-row parent-only instrument. The separate K3 call-free owner and role audit are also GREEN for all 36 route packets with zero model, evaluator, or reference calls. Twelve packets are physically ineligible and one row lacks a context treatment-control pair; exactly six non-scoring physical host canaries are now required before any nine-task screen |
...
`k3_six_route_non_scoring_physical_host_canaries_v1`, last closed task 26, and the wall
`call_free_k3_route_preflight_is_green_and_role_audited_but_six_largest_eligible_route_prompts_have_not_yet_completed_the_non_scoring_physical_host_canaries`.
The next legal action is
`build_prospectively_seal_and_role_audit_one_generic_non_scoring_host_canary_owner_that_runs_exactly_the_largest_physically_addressable_prompt_for_each_of_the_six_frozen_routes_through_the_same_TMax_tokenizer_decoder_wrapper_and_host_interlock_to_explicit_CANARY_OK_or_model_EOS_without_a_project_selected_quality_token_cap;_stop_on_any_physical_or_host_interlock_as_INVALID_HOST_OPERABILITY_OBSERVATION_and_do_not_open_the_nine_task_screen_until_all_six_receipts_are_audited_GREEN;_keep_Luna_omitted`.
