...
| Semantic-IR independent adequacy | `MATERIALIZATION_PREREGISTERED_ZERO_SOURCE_OR_MODEL_CALLS` | The powered design requires 18 distinct post-snapshot repositories, at least 13/18 overall and 2/3 in each of six edit strata. Metadata selection is GREEN for 18/18 exact PR, first-parent, changed-path, and license identities through a 90-request digest chain. A selected-file-only deterministic materializer is frozen and preflight-GREEN. Source bytes, archives, and evaluators remain unopened; network-source, model, Luna, training, D1, and D2 counters are zero |
