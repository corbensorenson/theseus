...
| Semantic-IR independent adequacy | `V3_DENOMINATOR_SEALED_RUNNER_SCORER_BINDING_REQUIRED` | The v2 resume preserved Task 1, completed fresh Tasks 2 and 3, and stopped before hidden evaluation when Task 4 call 2 reached the prospectively declared 600-second host watchdog. Seven of eight attempted calls completed normally; the watchdog-invalid call is not a capability or mechanism failure. Candidates 1–3 are sealed and must not be regenerated. Consumed Task 4 now has a fresh post-snapshot, BSD-3-Clause, source-disjoint `branch_or_predicate_replacement` from `scikit-bio/scikit-bio`; its exact source, evaluator, and parent-only packet are sealed. The v3 denominator preserves six completed calls and candidates 1–3, replaces only consumed Task 4, retains six balanced strata, and leaves Tasks 5–18 unchanged. The prospective runner, scorer, and classifier repair remain closed. Luna, teacher, training, D1, and D2 remain closed |
...
4. **Active:** preserve sealed candidates 1–3 and the complete v2 interruption
   evidence; repair watchdog-classifier precedence and prospectively bind the
   sealed v3 denominator to its runner and scorer before any new model call.
   Independently score
