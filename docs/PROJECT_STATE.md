...
| P4→D1 successor handoff | `FAIL_CLOSED_WAITING_FOR_EXACT_P4V2R2_SURVIVOR` | D1 now binds the exact P4-v2r2 disposition and TMax snapshot rather than the historical P4S/Qwen owners. A historical P4S status cannot activate it. Source membership remains unopened, all eight predecessor registries including P4-v2r2 are disjointness inputs, the 44-task cohort remains power-derived, and candidate/control calls stay closed until a fresh cohort and evaluator are sealed |
