...
| Active VCM claim | `VCM_V3_K2_03_V5_BUN_YARN_GREEN_TYPESCRIPT_UNRESOLVED` | Disposable Bun replay and Yarn online/offline replay are GREEN. TypeScript returned 2 without a host boundary, but its stdout diagnostic was not retained; Rust did not execute |
...
`k2_03_generic_ecosystem_risk_canaries_v5`, last closed task 26, and the wall
`typescript_nonzero_stdout_diagnostic_not_retained_and_rust_not_executed`.
The next legal action is
`seal_v6_zero_network_typescript_diagnostic_and_independent_rust_continuation`.
