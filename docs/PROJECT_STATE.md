...
| Semantic-IR independent adequacy | `TASK11_REPLACEMENT_METADATA_GREEN_SOURCE_REPLAY_PENDING` | The task-11 construct failure and initial sandbox transport pause are preserved. The amended 18-task registry independently replayed GREEN in 90 public metadata calls: exact PR identity, nondegenerate base/head revisions, changed-source membership, post-snapshot timing, distinct repositories, and license evidence all pass. Source replay and renewed archive/construct audits remain required before evaluator admission. Model, Luna, training, D1, and D2 counters remain zero |
