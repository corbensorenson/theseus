...
| Semantic-IR independent adequacy | `V3_MATERIALIZATION_PREREGISTERED_ZERO_EVALUATOR_OR_MODEL_CALLS` | The powered 18-repository design and six strata are unchanged. Two source-only task-4 failures are preserved: an unchanged selected path with a corrected 36-to-34 counter overcount, then proof that the rebased PR's `merge_commit_sha` identifies a docs-only final commit rather than the PR-head source repair. The prospective public PR `base.sha` to `head.sha` policy now has 18/18 GREEN nondegenerate metadata pairs with merge lineage retained separately, and v3 materialization preflight is GREEN with zero calls. Evaluator, model, Luna, training, D1, and D2 counters remain zero |
