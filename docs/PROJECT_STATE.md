...
| Semantic-IR independent adequacy | `V2_MATERIALIZATION_PREREGISTERED_ZERO_EVALUATOR_OR_MODEL_CALLS` | The powered 18-repository design and six strata are unchanged. Initial materialization failed closed on task 4's unchanged selected path and exposed a 36-versus-34 archive counter overcount; both the RED receipt and audit are preserved. The versioned `cli.py` amendment passed renewed 18/18 metadata acquisition through a fresh 90-request digest chain. A v2 materialization contract binds that result and the prior failure before renewed source retrieval. Evaluator, model, Luna, training, D1, and D2 counters remain zero |
