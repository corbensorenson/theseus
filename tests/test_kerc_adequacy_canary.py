...
def fixture_stage() -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        kerc_verifier_labels=np.tile(np.ones(4, dtype=np.float32), (8, 1)),
    )
...
def gate_fixture() -> dict:
    return {
        "acceptance": {
            ...
            "minimum_residual_accuracy": 0.5,
            "minimum_verifier_bit_accuracy": 0.75,
...
def test_failed_learning_is_inconclusive_not_architecture_falsification() -> None:
    state, gates = classify_gates(
        ...
        after={"mean_total_loss": 9.0, "token_accuracy": 0.0, "residual_accuracy": 0.25, "verifier_bit_accuracy": 0.5},
...
def test_lifecycle_failure_is_red() -> None:
    state, _gates = classify_gates(
        gate_fixture(),
        before={"mean_total_loss": 10.0, "token_accuracy": 0.0},
        after={"mean_total_loss": 1.0, "token_accuracy": 0.5, "residual_accuracy": 1.0, "verifier_bit_accuracy": 1.0},
        first_phase={"target_tokens_per_second": 2.0},
        second_phase={"target_tokens_per_second": 2.0},
        reload_delta=0.0,
        resume_delta=0.0,
