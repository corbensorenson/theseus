...
def test_p2b_pool_binds_frozen_instrument_task_and_blind_evaluator() -> None:
    pool = json.loads((ROOT / "configs" / "theseus_p2b_task_pool.json").read_text(encoding="utf-8"))
    ...
    assert pool["instrument_freeze_commit"] == "d616ff2c"
...
def test_p2b_task_uses_one_repo_relative_namespace_and_retains_upstream_archives() -> None:
    task = json.loads((ROOT / "configs" / "theseus_p2b_task_requests_7502.json").read_text(encoding="utf-8"))
    evaluator_config = json.loads(
        (ROOT / "configs" / "theseus_p2b_evaluator_requests_7502.json").read_text(encoding="utf-8")
    )

    assert task["source_archive_root"] == "psf-requests-661970d"
    assert task["allowed_effect_paths"] == ["src/requests/models.py"]
    assert all(not path.startswith(task["source_archive_root"] + "/") for path in task["allowed_effect_paths"])
    assert task["source_provenance"]["license_spdx"] == "Apache-2.0"
...
def test_p2b_task_is_source_disjoint_and_never_training_or_claim_evidence() -> None:
    task = json.loads((ROOT / "configs" / "theseus_p2b_task_requests_7502.json").read_text(encoding="utf-8"))
    screen = task["contamination_screen"]

    assert screen["public_benchmark"] is False
    assert screen["previous_theseus_surface"] is False
    assert screen["source_disjoint_from_p2a"] is True
