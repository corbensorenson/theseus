...
| Verification bandwidth | VCM/context-governor and verifier-spine records exist, and Octopus route decisions now carry verifier-capacity budgets, residual-obligation ledgers, escalation thresholds, and governance-tax receipts. | Extend the same accounting into fanout, STS, verifier-cascade, and generator-ranking runs before route-policy changes. |
...
- A fast path cannot be promoted on throughput if accepted verified output gets
  worse.
- Octopus route decisions emit verification-bandwidth receipts with obligation
  counts, verifier capacity, decomposition contract, residual obligations, and
  escalation thresholds.
- Octopus route decisions emit governance-tax receipts with per-gate overhead,
  review load, caught route risk, and tax-per-caught-risk accounting.
...
| 16 | MoECOT And Octopus Router Integration | Partial | Octopus route decisions now carry verifier-capacity budgets, residual-obligation ledgers, escalation thresholds, governance-tax overhead, review load, and tax-per-caught-risk accounting. Extend the same receipts into fanout/STS/verifier-cascade runs and gather more diverse real task-to-arm traces before major route-policy changes. |
