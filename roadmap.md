...
|---:|---|---|---|
| 1 | Kernel English + hierarchical residual compiler | `partial` / `INCONCLUSIVE_EXPERIMENT` | faithful learned mechanics, VCM interaction binding, lifecycle, source-replayed multi-turn/multiple-valid supervision, explicit clarify/abstain decisions, narrow source-grounded questions, and train-only contextual FrameNet ambiguity are GREEN; add unresolved-ambiguity/broad-entailment and protected-span abstention coverage, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any utility verdict |
...
| Kernel English + hierarchical residual compiler | Phases 3, 10, 13, 14, 16 | `partial` / `INCONCLUSIVE_EXPERIMENT`: exact packets/VCM residual mechanics feed an explicit MLX KERC model with trusted stage routing, content-bound V_K/V_P codebooks, disjoint Kernel/pointer/surface support, manual FrameNet segment/token supervision, bounded VCM interaction state, learned four-channel allocation, and an independent five-dimension verifier. The independently replayed 11,424-record corpus yields a GREEN 26,360-view stage, including 1,064 multi-turn OASST2 records, 2,128 reviewed source-bound answers, a reserved 24-record explicit clarification/abstention stratum, and 96 source-disjoint Dolly questions with unique exact answer support across six required question forms. A train-only contextual FrameNet prior exposes at least two human-annotated senses for 59 lexical units and binds typed alternatives to 411 train, 178 development, and 157 evaluation records; heldout labels cannot influence the prior. This supports contextual frame selection only, not unresolved ambiguity, calibrated probability, WordNet equivalence, broad entailment, truth, completeness, or learned utility. The prior 64-update canary covers the original four verifier dimensions; a new eight-step canary covers all 19 decision/residual/verifier groups with zero omissions but grants no quality credit. Strict reload, resume, migration, rollback, cleanup, and sustained Metal mechanics pass. Dialogue/decision utility and calibration remain unclaimed. The prior keyword/linear result remains proxy-negative only | add unresolved-ambiguity/broad-entailment labels and protected-span abstention, train context counterfactuals, calibrate decisions/verifier behavior, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any KERC utility or failure claim |
...
use, not that prior context improves answers. A fresh eight-step MLX canary uses a
content-hashed deterministic set-cover prefix and observes all 19 required objective,
normal/clarify/abstain, interaction, residual, and five-dimension verifier groups in
789 target positions. Its checkpoint remains `NOT_EVALUATED`; this is mechanics
evidence, not quality or calibration. The new OASST2 rows establish source-bound
multi-turn, multiple-valid, and explicit-decision supervision, while the Dolly question
  ...
  confidence, or learned question-answer quality. Unresolved ambiguity/broad-entailment and protected-span abstention,
trained context counterfactuals, calibrated verifier behavior, full-size multi-seed
