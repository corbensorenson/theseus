...
7. Claude's later book-mining pass does not add another lane; it sharpens the
   ...
   `0` behavior passes. The current wall is therefore stronger learned
   state-transition/body construction: the model still over-opens nested guard
   blocks and fails to produce a closed admissible update/finalizer/return
   body.
