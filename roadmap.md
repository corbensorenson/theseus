...
| `T1` Selected neural-seed campaign | active at step 9,048; prospectively anchored; not evaluated | The shared trunk has consumed 69,310,840 of 1,096,734,920 frozen pretraining positions. Its exact model, AdamW, RNG, and receipt identities are bound by `configs/neural_seed_t1_lineage_anchor.json`; every later segment must join an append-only before/after ledger before another launch. The anchor honestly records that the complete step-3,480-to-9,048 predecessor chain is unavailable, so later evidence can claim only the exact prospective lineage it earns. Continue the selected practical MoECOT lane and matched dense-active/dense-total controls under the 64-step compiled FP32 contract. No capability, source-disjoint behavior, T2 numerator, or support movement has been observed. |
...
- Current run: the shared trunk is at checkpoint step 9,048 and 69,310,840 of
  1,096,734,920 frozen pretraining positions. Its exact model, AdamW, RNG, and receipt
  identities are bound by `configs/neural_seed_t1_lineage_anchor.json`. The historical
  ...
  retroactively negate the committed T0A architecture qualification. Before another
  state-changing segment launches, the live receipt must match the prospective anchor or
...
5. **Operate the resource-aware segment scheduler.** Never suspend an in-flight Metal
   ...
   **Current execution evidence (2026-07-28):** after one launch correctly stopped under
   concurrent 3.2 GB repository-validation pressure, the exact clean-v2 scratch lineage
   restarted from canonical step 3,480 and published four 64-step transactions at
   `2,817.693`, `2,844.904`, `3,021.339`, and `3,089.663` joined positions/second. The
   scratch receipt reached step 3,736 / 28,629,458 optimizer positions while the canonical
   model `af4b5524...9247`, optimizer `29bcb5f8...a2f7`, RNG
   `9209383f...850f`, and receipt `3f4c4ced...007a` remained unchanged. AC power changed
   after the fourth segment, so the next availability check stopped before segment five.
   That mixed-condition window is durable but nonqualifying. The report state machine now
   labels an after-segment availability transition `PAUSED`, records why the current window
   is nonqualifying, and archives it on resume instead of reporting a false training
   failure. Continue the same scratch lineage only when AC power is stable; obtain two
   adjacent three-segment same-condition windows and the independent exact replay before
...
`q_proj`. Public Core ML executes forward and input-gradient matmuls on ANE-preferred
plans; single-thread Accelerate computes FP32 `dW`; compiled MLX/Metal computes the
half-MSE station loss, gradient clipping, and one FP32 AdamW update. All results carry one
authoritative weight generation.

Across 64 optimizer updates, output, loss, `dX`, `dW`, and updated-weight parity all pass
their preregistered tolerances. Maximum absolute deltas are `0.000610352`,
`0.000000537`, `0.000000477`, `0.00000000490`, and `0.000057006`, respectively.
All tensors remain finite, generation 64 is reached exactly, step-32 save/reload is exact,
and one-step replay is exact. A guarded rerun peaks at `340.141 MiB` process RSS, preserves
at least `3,642.562 MiB` reclaimable memory, and grows swap by `0.0 MiB`.

The route nevertheless loses decisively on the metric that matters. The public hybrid
averages `10.068 ms` per joined station update while matched compiled MLX averages
`4.941 ms`: `0.490742x` control-over-hybrid speedup, or a `2.037730x` hybrid slowdown.
Mandatory hot-step Python and NumPy bridges are present. Therefore:
...
14. The native Metal/Accelerate block remainder is now GREEN from the frozen hidden
    and attended boundaries. It covers out projection, both unscaled residuals, the
    second RMSNorm forward/backward, SwiGLU forward/backward, down projection, the
    masked scalar objective, both returned boundary gradients, all five remainder
    parameter leaves (`2,621,952` parameters), one global clip, and one FP32 AdamW
    publication. Every gradient element is nonzero and finite; update replay is
    byte-exact; 64 successive updates remain finite; Metal activation and gate/up
    gradient deltas are at most `4.768e-7`, `4.657e-10`, and `4.657e-10` with zero
    registered mismatches. Guarded mean joined wall is `6.678 ms`, peak process and
    inferred unified memory are `171.062 MiB`, minimum reclaimable memory is
...
Selection remains critical-path based. There is no fixed device fraction, clock-of-day
window, arbitrary percentage hurdle, or requirement to keep idle hardware busy. A device
...
8. **Adequacy-sized canaries.** Replace the global eight-step architecture-canary assumption
   ...
   green, and the optimizer-matched adequacy child started but was stopped by the external guard
   after reclaimable memory fell below its 4,096 MiB live reserve. Maximum inferred unified memory
   was 1,529.766 MiB, maximum process RSS was 231.75 MiB, and swapout growth was zero. The
   replacement freeze therefore remains unpublished.
