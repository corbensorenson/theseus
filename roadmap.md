...
|---:|---|---|---|
| 1 | Kernel English + hierarchical residual compiler | `partial` / K0 GREEN; K1a-K1j source/replay/concept-registry evidence GREEN; K1k scoped-semantic ABI, exact serialization, full corpus migration, and bounded parallel admission GREEN; K1l bounded human eRST-to-scope supervision and learned-view projection GREEN; full current-corpus canonical stage RED; broader construct validity RED; utility unclaimed | preserve the exact packet/codec/lifecycle work, but do not call the current four-channel head a learned KERC allocator. K1e preserves 13 complete manual event-coreference groups with 215 aligned mentions; K1f preserves 846 complete MPQA attribution graphs; K1h preserves 9,785 licensed eRST neighborhoods; K1i preserves 5,361 source-declared entity/coreference records and 7,180 concept capsules; K1j adds exact ambiguity-preserving lookup over 218,061 WordNet-derived relations. K1k adds exact proposition/scope ownership for 14 operators, independent structural reconstruction, five causal scope/role/quantifier interventions, four rejected topology/authority mutations, strict typed values, and `KE-SERIALIZATION-2.0` decode-and-compare replay. K1l admits 1,525 source-grounded two-proposition rows across seven conservative eRST mappings, preserves 90 mapped multi-edge neighborhoods as explicit exclusions, independently reconstructs direction/roles/spans, and proves exact scope programs reach the compiler target and reasoner input without target leakage. Train/dev/eval counts are 1,127/204/194 with minimum relation counts 34/6/9 and zero source-group overlap; dev/eval retain five missing relation-by-genre cells each. All 27,941 corpus records are independently admitted with zero failures. The latest selective four-worker replay takes 658.590s with 18,156 semantic hits and 9,785 independently recomputed GUM discourse rows. The first full current-corpus stage audit closes explicit semantic-tag and composite-frame learned-ABI defects, selects all requested 21,209 records, and writes 44,943 representable views, but remains RED after 5,519.207s: hash-only sampling misses feasible objective/grounded quotas, two raw sources cross splits, and 1,232 positive plus four negative views exceed the 8,192-token contract under the verbose model ABI. Treat this as `INCONCLUSIVE_IMPLEMENTATION`, not a negative result. Next implement deterministic quota-aware source-disjoint selection, exact compact/invertible program and answer transport or faithful hierarchical long-record processing, and content-bound incremental replay before continuing K2. PMB remains calibration-only. The corpus still does not supply complete proposition semantics, learned scope compilation, truth, temporal truth, natural-context learned cross-document linking, or utility. Complete K1-K8 in order: finish decision-grade source coverage; per-unit economics; real interaction amortization; coordinated learned compiler/core/renderer; construct-valid verifier; false-negative controls; then one prospectively frozen matched campaign. No mechanics canary, schema replay, or proxy score may support a KERC utility or failure claim |
...
| Kernel English + hierarchical residual compiler | Phases 3, 10, 13, 14, 16 | `partial`: `KPP-1.1`, exact object handling, VCM state replay, separate code spaces, shared-trunk stage adapters, arithmetic-code receipts, development-only lambda selection, and checkpoint/migration mechanics are real. K1a-K1j preserve bounded independently replayed frame, decision, event-coreference, attribution, discourse, entity/coreference, and concept-registry structure. K1k gives nested propositions exact scope ownership across 14 explicit operators and exact full-program serialization. K1l now grounds six represented scope operators in 1,525 licensed human eRST examples across seven direction-sensitive relation mappings, independently reconstructs every admitted program, excludes 90 shared-endpoint multi-edge neighborhoods rather than inventing ownership, and carries each exact scope graph into the canonical compiler target and reasoner input. This still does not close construct validity: only bounded two-proposition relation families are authorized; five dev and five eval relation-by-genre cells remain missing; complete proposition semantics, learned scope compilation, truth, natural-context cross-document linking, correction lattices, learned macros, interaction-global dictionaries, and per-unit allocation remain open. The residual head still learns channel presence, the corpus remains economically negative, and no learned utility, amortization, efficiency, SOTA, AGI, or ASI claim is authorized | execute K0-K8 in order. Finish source-grounded K1 coverage without benchmark training; implement per-unit constrained economics and genuine multi-turn state; complete coordinated objectives and independent verification; only then freeze and train the matched KERC/control campaign. Preserve current reports as mechanics evidence and proxy negatives, not as a verdict |
...
2. **K1 - decision-grade semantic representation and corpus.** Replace wrapper-shaped
   ...
   **K1l source-grounded scoped supervision (`GREEN` source evidence; full stage `RED`,
   ...
   rows. The first unchanged full canonical-stage replay then selected the configured
   `14,238/3,621/3,350` records and wrote `44,943` representable views in `5,519.207s`, but
   correctly remained RED. Five feasible objective-authority floors and the grounded quota
   were missed by hash-only selection, two duplicate raw sources crossed splits, and the
   verbose learned program/answer transport forced `1,232` positive plus four verifier-
   negative views past the `8,192`-token contract. No row was silently truncated and no
   floor was lowered. This is `INCONCLUSIVE_IMPLEMENTATION`; repair constrained selection,
   exact compact or hierarchical learned transport, and content-bound incremental replay
   before K2 or training. This is bounded
   source-grounded scope supervision and learned-view projection evidence, not complete sentence
   semantics, truth, temporal truth, learned competence, utility, SOTA, AGI, or ASI.
