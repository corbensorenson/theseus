...
|---:|---|---|---|
| 1 | Kernel English + hierarchical residual compiler | `partial` / K0-K1l corpus, protocol, source-grounding, canonical materialization, and exact replay GREEN; broader construct validity RED; learned utility unclaimed | The canonical stage validates all 27,941 candidates and deterministically selects 14,238/3,621/3,350 source-disjoint train/dev/eval records. It emits 53,414 learned views, 53,414 generator-loss-disabled verifier corruptions, and 384 context counterfactuals from 18,033 unique raw sources. Exact typed compact transports, iterative eight-node compiler chunks, topologically scheduled eight-node reasoner chunks, and length-only ragged 8K/16K buckets produce zero truncation, drop, fallback, public rows, or external inference. Every frozen objective floor and all 96 grounded rows are retained. The cold stage takes 1,636.573s and exact content-addressed replay takes 1.58s. This closes materialization, not capability: learned compiler/core/renderer competence, per-unit allocation, interaction amortization, calibrated semantic verification, broader proposition/truth semantics, and utility remain open. Next run bounded learnability/gradient/overfit/checkpoint/reload/resume/intervention canaries, repair any failed owner, then complete K2-K7 and freeze the matched multi-seed K8 campaign. No mechanics result may support a utility or failure claim. |
...
| Kernel English + hierarchical residual compiler | Phases 3, 10, 13, 14, 16 | `partial`: governed architecture/data materialization is GREEN and exact-replayable, with compact hierarchical causal routes and zero truncation/drop. It remains untrained and therefore proves no learned semantic competence, utility, efficiency, or architecture advantage. Broader construct coverage, learned per-unit residual allocation, real VCM interaction amortization, calibrated neural verification, correction/macros/PARTIAL behavior, and source-disjoint functional evidence remain open | execute the bounded mechanics qualification on the actual canonical stage, then complete K2-K7 and prospectively freeze the strong matched multi-seed K8 comparison. Repair inadequate mechanisms as `INCONCLUSIVE_*`; never retire the idea from a weak proxy |
