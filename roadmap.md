...
| 10 | Practical Neural Seed Survival Lane | Partial | Body-transition and prompt-operation conditioning now emit integrity-clean broad4 candidates without public/external/fallback credit; the newest strict operation smoke reaches nontrivial-return rate 1.0 but behavior remains 0/4, so the target is private verifier-positive learned operation/state-transition semantics, not another renderer or selector. |
