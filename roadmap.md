...
|---:|---|---|---|
| 1 | Kernel English + hierarchical residual compiler | `partial` / mechanics GREEN, utility unclaimed | faithful learned mechanics, VCM interaction binding, lifecycle, source-replayed multi-turn/multiple-valid supervision, explicit clarify/abstain decisions, narrow source-grounded questions, train-only contextual FrameNet ambiguity, and verifier-only withheld/shuffled context interventions are integrated. Verifier-only rows have zero residual authority; the stage-conditioned same-model rung learns all four residual channels to 1.0 and retains 1.0 after canonical joint token/verifier/residual updates. The cold 64-update mixed score remains a diagnostic only. Add unresolved-ambiguity/broad-entailment and protected-span abstention coverage, calibrate verifier decisions, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any utility verdict |
...
| Kernel English + hierarchical residual compiler | Phases 3, 10, 13, 14, 16 | `partial` / mechanics GREEN, utility unclaimed: exact packets/VCM residual mechanics feed an explicit MLX KERC model with trusted stage routing, content-bound V_K/V_P codebooks, disjoint Kernel/pointer/surface support, manual FrameNet segment/token supervision, bounded VCM interaction state, learned four-channel allocation, and an independent five-dimension verifier. The independently replayed 11,424-record corpus yields a GREEN 26,360-view stage with 26,360 primary verifier corruptions plus 384 verifier-only withheld/shuffled context interventions, including 1,064 multi-turn OASST2 records, 2,128 reviewed source-bound answers, a reserved 24-record explicit clarification/abstention stratum, and 96 source-disjoint Dolly questions with unique exact answer support across six required question forms. Context donors stay source-disjoint within split, exclude the original answer, rebuild program/answer checksums, and receive zero generator or unique-data credit. A train-only contextual FrameNet prior exposes at least two human-annotated senses for 59 lexical units and binds typed alternatives to 411 train, 178 development, and 157 evaluation records; heldout labels cannot influence the prior. This supports contextual frame selection and counterfactual support sensitivity only, not unresolved ambiguity, calibrated probability, WordNet equivalence, broad entailment, truth, completeness, context utility, or learned utility. The current cold-start Apple-MLX canary covers all 21 objective/decision/interaction/residual/verifier groups with 16 real rows and zero omissions. Token accuracy reaches 0.354244 and verifier macro balanced accuracy 0.784569 with 1.0 minimum negative recall; residual macro balanced accuracy remains 0.5 after 64 mixed updates and is retained as a diagnostic only. Verifier-only rows have zero residual authority. The source-and-stage-conditioned same-model learnability rung records nonzero head/source gradients, reaches 1.0 on all four channels within 160 updates, and retains 1.0 after 64 canonical joint updates. Reload, resume, migration, rollback, cleanup, causal interventions, trained ablations, and sustained Metal mechanics pass; the canary grants no utility or negative-verdict authority. Dialogue/context utility and decision calibration remain unclaimed. The prior keyword/linear result remains proxy-negative only | add unresolved-ambiguity/broad-entailment labels and protected-span abstention, calibrate decisions/verifier behavior, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any KERC utility or failure claim |
...
`0.354244`; verifier macro balanced accuracy is `0.784569` with `1.0` minimum negative
...
balanced accuracy on every channel within 160 bounded updates. Explicit trusted-stage
conditioning prevents long sources from washing out the stage signal, and all four
channels retain `1.0` after 64 additional canonical joint updates. The complete mechanics
