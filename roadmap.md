...
|---:|---|---|---|
| 1 | Kernel English + hierarchical residual compiler | `partial` / `INCONCLUSIVE_EXPERIMENT` | faithful learned mechanics, VCM interaction binding, lifecycle, source-replayed multi-turn/multiple-valid supervision, explicit clarify/abstain decisions, narrow source-grounded questions, train-only contextual FrameNet ambiguity, and verifier-only withheld/shuffled context interventions are integrated. Verifier-only rows now have zero residual authority; the same-model residual-only rung proves all four channels learn to 1.0 with nonzero head and source gradients, while the 64-step joint objective remains at 0.5. Add unresolved-ambiguity/broad-entailment and protected-span abstention coverage, implement prospectively specified multi-objective scheduling under the same frozen rows, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any utility verdict |
...
| Kernel English + hierarchical residual compiler | Phases 3, 10, 13, 14, 16 | `partial` / `INCONCLUSIVE_EXPERIMENT`: exact packets/VCM residual mechanics feed an explicit MLX KERC model with trusted stage routing, content-bound V_K/V_P codebooks, disjoint Kernel/pointer/surface support, manual FrameNet segment/token supervision, bounded VCM interaction state, learned four-channel allocation, and an independent five-dimension verifier. The independently replayed 11,424-record corpus yields a GREEN 26,360-view stage with 26,360 primary verifier corruptions plus 384 verifier-only withheld/shuffled context interventions, including 1,064 multi-turn OASST2 records, 2,128 reviewed source-bound answers, a reserved 24-record explicit clarification/abstention stratum, and 96 source-disjoint Dolly questions with unique exact answer support across six required question forms. Context donors stay source-disjoint within split, exclude the original answer, rebuild program/answer checksums, and receive zero generator or unique-data credit. A train-only contextual FrameNet prior exposes at least two human-annotated senses for 59 lexical units and binds typed alternatives to 411 train, 178 development, and 157 evaluation records; heldout labels cannot influence the prior. This supports contextual frame selection and counterfactual support sensitivity only, not unresolved ambiguity, calibrated probability, WordNet equivalence, broad entailment, truth, completeness, context utility, or learned utility. The current 64-update Apple-MLX canary deterministically covers all 21 objective/decision/interaction/residual/verifier groups with 16 real rows and zero omissions. It is honestly `INCONCLUSIVE_EXPERIMENT`: token accuracy reaches 0.352399 and verifier macro balanced accuracy 0.776876 with 1.0 minimum negative recall, while residual macro balanced accuracy is 0.5 under the short joint objective. Verifier-only rows have zero residual authority; a separate same-model, same-row residual-only rung records nonzero head/source gradients and reaches 1.0 on all four channels within 256 updates, proving learnability but not utility. Reload, resume, migration, rollback, cleanup, causal interventions, trained ablations, and sustained Metal mechanics pass; the canary grants no utility or negative-verdict authority. Dialogue/context utility and decision calibration remain unclaimed. The prior keyword/linear result remains proxy-negative only | add unresolved-ambiguity/broad-entailment labels and protected-span abstention, implement prospectively specified gradient/exposure-balanced joint scheduling without changing the frozen evidence, calibrate decisions/verifier behavior, freeze total-cost controls, and run the multi-seed source-disjoint matched campaign before any KERC utility or failure claim |
...
stage is GREEN with 26,360 authorized views and 26,360 verifier corruptions over 11,424
...
counterfactual support sensitivity. Under this expanded objective, the current 64-update
Apple-MLX canary is honestly `INCONCLUSIVE_EXPERIMENT`: its deterministic 16-row subset
...
`0.352399`; verifier macro balanced accuracy is `0.776876` with `1.0` minimum negative
recall; but all four informative residual channels remain at `0.5` macro balanced
accuracy under the short joint objective. Verifier-only and context-counterfactual rows
...
balanced accuracy on every channel within 256 bounded updates. The mechanism is learnable;
prospectively specified joint-objective scheduling is now the mechanics repair target,
not evidence against KERC. Checkpoint and
...
protected-span abstention, joint-objective scheduling, calibrated verifier behavior, full-size multi-seed
