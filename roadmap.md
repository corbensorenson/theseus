...
| `T1` — Frozen neural-seed campaign | `running_shared_trunk` | The exact campaign reached step 75 before an operator stop with no checkpoint or capability credit. Native MLX GQA is parity-tested; the relaunch is healthy through step 50 at 741 target positions/second overall, about 792 over steps 26-50, with finite declining loss. Preserve the exact run through checkpoint/resume, qualify faster numerical or hardware routes separately, then train the five arms and two matched dense controls with complete lineage. | Frozen functional evaluation. |
...
remains unconsumed and capability remains unmeasured. The full relaunch reached step 50
at 741 positions/second overall and about 792 over its second 25-step interval. That is
a real gain over the prior 619-position/second route, but it projects to roughly 16-17
days for the shared trunk on this M1 and supersedes the short canary as the runtime basis.
...
| Generator capability (held-out utility) | Phase 10 | the 10.8M regime remains closed at 0/160. The corrected 57.340M-active MoECOT arm and 57.349M/67.357M dense controls pass full MLX update, checkpoint/reload, optimizer-resume, data-lineage, and resource canaries. The exact 57M relaunch is healthy through step 50 at 741 positions/second overall, but has not reached its first checkpoint. A fresh source-disjoint 160-case functional surface is frozen and unconsumed. | preserve the unchanged run through first checkpoint/resume while qualifying a faster route separately; evaluate all completed candidates without tuning from heldout outcomes and publish the scoped verdict |
...
- The practical model lane is now executable-authorized at 57.340M active parameters.
  ...
  compile and batch-24 alternatives were measured and rejected on this M1. The full
  relaunch is healthy through step 50 at 741 positions/second overall and about 792 over
  steps 26-50, so the short canary does not set the sustained projection. KERC remains
  banked and deferred. Learned qualification of STS, VCM, search, preference/RL, and fast
  generation still waits for a nonzero model-only numerator.
