...
- STS/ranker policy runs emit the same verification-bandwidth and
  governance-tax receipts before route-policy adoption; missing or stale
  candidate manifests must surface as no-admissible residuals, not stale GREEN
  selector claims.
...
| 16 | MoECOT And Octopus Router Integration | Partial | Octopus route decisions, train-once fanout, private verifier-cascade, and STS ranker policy runs now carry verifier-capacity budgets, residual-obligation ledgers, escalation thresholds, governance-tax overhead, review load, and tax-per-caught-risk accounting. The refreshed STS ranker rerun is honestly YELLOW because the old candidate manifests are absent in the cleaned local state, yielding `no_admissible_rate=1.0`; next, regenerate current STS candidate manifests through the registered fanout path and gather more diverse real task-to-arm traces before major route-policy changes. |
