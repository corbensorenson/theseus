...
| Correctness training (DPO->GRPO/RLVR) | Track 3 / Phase 10 | learned optimization remains premature at the zero-pass floor; the shared seven-target update-lease mechanics are wired, while objective/reference/reward/checkpoint interfaces remain pre-training requirements | wire and canary the complete objective family now; run and select it only after a behavior-positive proposer creates honest verifier signal |
...
| Policy optimization (DPO/GRPO/RLVR) | shared update authority is now wired across seven policy targets with heldout, cost, journal, sentinel, conflict, and exact rollback controls; objective-specific adapters and learned efficacy remain incomplete | 7, 10 |
