#!/usr/bin/env python3
"""Independently rederive task 36's retained Cargo dependency closure."""
...
POLICY="project_theseus_vcm_task36_dependency_closure_audit_v1";DEFAULT_CONFIG=ROOT/"configs"/"theseus_vcm_task36_dependency_closure_audit.json"
def main()->int:
...
def audit(path:Path=DEFAULT_CONFIG)->dict[str,Any]:
 ...
 if canary.get("trigger_state")!="GREEN" or canary.get("state")!="TASK_36_EXACT_CARGO_DEPENDENCY_CACHE_ACQUIRED_AND_OFFLINE_REPLAY_QUALIFIED":faults.append("canary_not_green")
 if ccfg.get("policy")!="project_theseus_vcm_task36_dependency_canary_v1":faults.append("canary_config_invalid")
 ...
 command_checks={"online_returncode_zero":online.get("returncode")==0,"offline_returncode_zero":offline.get("returncode")==0,"online_boundary_clear":online.get("boundary_hit") is False,"offline_boundary_clear":offline.get("boundary_hit") is False,"online_network_phase":online.get("network_denied") is False,"offline_deny_network_phase":offline.get("network_denied") is True,"online_command_exact":p2a.strings(online.get("command"))[1:]==online_args,"offline_command_exact":p2a.strings(offline.get("command"))[1:]==offline_args}
 ...
 store_checks={"checksum_denominator_exact":len(checks)==32 and check_identity==p2a.mapping(ccfg.get("task")).get("expected_checksum_identity_sha256"),"all_crates_matched":store_receipt.get("matched_checksum_count")==store_receipt.get("expected_checksum_count")==32,"no_missing_crates":store_receipt.get("missing_packages")==[],"bytes_match":store_receipt.get("bytes")==reported.get("bytes"),"files_match":store_receipt.get("file_count")==reported.get("file_count"),"identity_matches":store_receipt.get("files_identity_sha256")==reported.get("files_identity_sha256"),"crate_receipts_match":store_receipt.get("crate_receipts_sha256")==reported.get("crate_receipts_sha256")}
 ...
 online_cache=p2a.mapping(receipts.get("online_cache"));offline_cache=p2a.mapping(receipts.get("offline_cache"));phase_checks={"online_faults_empty":online_cache.get("faults")==[],"offline_faults_empty":offline_cache.get("faults")==[],"online_offline_cache_exact":{k:online_cache.get(k) for k in ("expected_checksum_count","matched_checksum_count","missing_packages","crate_receipts_sha256","file_count","bytes","files_identity_sha256")}=={k:offline_cache.get(k) for k in ("expected_checksum_count","matched_checksum_count","missing_packages","crate_receipts_sha256","file_count","bytes","files_identity_sha256")}};faults.extend(f"phase_check_failed:{k}" for k,v in phase_checks.items() if not v)
 ...
 obs={"task_index":36,"lock_checksum_package_count":len(checks),"matched_checksum_count":store_receipt.get("matched_checksum_count"),"retained_store_bytes":store_receipt.get("bytes"),"retained_store_file_count":store_receipt.get("file_count"),"source_file_count":source.get("file_count"),"source_bytes":source.get("bytes"),"current_free_bytes":current,**{k:0 for k in zero}}
 return {"policy":POLICY,"created_utc":p2a.now(),"trigger_state":"GREEN" if not faults else "RED","state":"TASK_36_DEPENDENCY_CLOSURE_INDEPENDENTLY_REDERIVED" if not faults else "TASK_36_DEPENDENCY_CLOSURE_AUDIT_FAILED","faults":sorted(set(faults)),"config":base.identity(path),"artifacts":{n:base.identity(p) for n,p in paths.items()},"retained_store":p2a.rel(store),"command_checks":command_checks,"store_checks":store_checks,"source_checks":source_checks,"phase_checks":phase_checks,"storage_checks":storage_checks,"rederived_store":store_receipt,"rederived_source":source,"observations":obs,"static_audit_only":True,"network_or_dependency_execution_performed":False,**{k:0 for k in zero},"maximum_inference":cfg.get("maximum_inference")}
