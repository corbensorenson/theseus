...
def preflight(path:Path=DEFAULT_CONFIG)->tuple[dict[str,Any],dict[str,Any],list[str]]:
 ...
 for row in rows:
  ...
  if row.get("manager") not in {"uv","cargo_static"}:faults.append(f"task_{index}:manager_invalid")
  ...
  bound[index]={"row":row,"target":target,"archive":archive,"root":root,"manifests":manifests}
 authority=p2a.mapping(cfg.get("authority"));allowed={"target_archive_extraction_authorized","registry_metadata_resolution_authorized","immutable_lock_write_authorized","shared_resolver_cache_authorized","repository_owned_static_lock_copy_authorized"}
 ...
 if cache!=(ROOT/"runtime/vcm_evaluator/dependency_store/replacement-resolution-v1").resolve():faults.append("shared_cache_root_invalid")
