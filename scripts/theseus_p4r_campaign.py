#!/usr/bin/env python3
"""Run the sealed completion-based P4R campaign exactly once."""
...
POLICY = "project_theseus_p4r_cognitive_compilation_campaign_v1"
...
POOL_SEAL_COMMIT = "79db08bab96e2f2fe5bb8bbf235b86776113614a"
POOL_SHA256 = "789a37f95ad14049cab076b0950d6711dbd2278234941dd131d6bc501a863813"
INSTRUMENT_FREEZE_COMMIT = "0a8f0bec2cd8883fa6b1176e5f9979554c45539f"
...
def run_campaign() -> dict[str, Any]:
    ...
    for row in p2a.dicts(pool.get("tasks")):
        ...
        if not paths["run"].is_file():
            ...
            run = p4r.run_experiment(INSTRUMENT, ROOT / str(row["task"]))
...
def audit_campaign(extra_faults: list[str] | None = None) -> dict[str, Any]:
    ...
    if pool.get("instrument_freeze_commit") != INSTRUMENT_FREEZE_COMMIT:
        faults.append("instrument_freeze_binding_invalid")
    repair = p2a.mapping(pool.get("post_selection_mechanics_repair"))
    if repair.get("candidate_or_control_calls_before_repair") != 0:
        faults.append("mechanics_repair_not_pre_generation")
    if repair.get("task_membership_changed") is not False:
        faults.append("mechanics_repair_changed_task_membership")
    ...
    instrument_audit = p4r.audit_instrument(INSTRUMENT)
    ...
    for expected, row in enumerate(rows, 1):
        ...
        if run_exists:
            ...
            if p2a.mapping(run.get("denominators")).get("project_selected_quality_token_cap") is not None:
            ...
            for call_index, termination in enumerate(telemetry, 1):
                ...
                if termination.get("safety_ceiling_hit") is True:
                    faults.append(f"physical_context_boundary_hit:{stem}:{call_index}")
                ...
                context_tokens = int(termination.get("model_context_window_tokens") or 0)
                ...
                if min(prompt_tokens, generated_tokens, context_tokens, effective) < 1:
                    faults.append(f"termination_token_custody_invalid:{stem}:{call_index}")
    ...
    return {
        ...
        "scope": "Exact sealed completion-based P4R local development campaign; no hosted, D1, D2, serving, training, or automatic book-support authority.",
        "pool": relative(POOL),
...
def result_paths(row: dict[str, Any]) -> dict[str, Path]:
    suffix = str(row.get("stem") or "").removeprefix("p4r_")
    return {
        "run": ROOT / "reports" / f"theseus_p4r_{suffix}_run.json",
        "evaluation": ROOT / "reports" / f"theseus_p4r_{suffix}_evaluation.json",
    }
