#!/usr/bin/env python3
"""Run the sealed mechanics-qualified P4S local campaign exactly once."""
...
POLICY = "project_theseus_p4s_cognitive_compilation_campaign_v1"
...
def run_campaign() -> dict[str, Any]:
    ...
    for row in p2a.dicts(pool.get("tasks")):
        ...
        if not paths["run"].is_file():
            ...
            run = p4s.run_experiment(INSTRUMENT, ROOT / str(row["task"]))
...
def audit_campaign(extra_faults: list[str] | None = None) -> dict[str, Any]:
    ...
    for key in (
        "local_model_calls", "hosted_model_calls", "teacher_calls",
        "deterministic_request_compiler_calls", "public_calibration_cases_consumed",
        "D1_cases_consumed", "D2_cases_consumed", "training_rows_written",
    ):
    ...
    instrument_audit = p4s.audit_instrument(INSTRUMENT)
    ...
    return {
        ...
        "scope": (
            "Exact sealed completion-based P4S local decision-development campaign; "
            "no hosted, D1, D2, serving, training, or automatic book-support authority."
        ),
        "pool": relative(POOL),
        ...
        "maximum_inference": (
            "Campaign execution custody only; terminal scientific status is computed "
            "separately from sealed blind evaluations."
        ),
    }
