...
def _result(
    case: dict[str, Any], *, passed: bool, stage: str, started: float, **extra: Any
) -> dict[str, Any]:
    return {
...
def _run_sandboxed(
    ...
    resolved = (
        shutil.which(executable) if not executable.startswith("/") else executable
    )
    ...
    wrapped = [
        ...
        "-p",
        _sandbox_profile(workdir, browser=browser),
        *command,
    ]
    ...
    try:
        ...
        protocol_lines = [
            ...
            and len(line.encode("utf-8", errors="replace"))
...
def _render_chrome(
    command: list[str], workdir: Path, screenshot: Path, timeout: int
) -> dict[str, Any]:
    env = {key: value for key, value in os.environ.items() if key in ALLOWED_ENV}
    env["TMPDIR"] = str(workdir)
    wrapped = [
        "/usr/bin/sandbox-exec",
        "-p",
        _sandbox_profile(workdir, browser=True),
        *command,
    ]
    started = time.monotonic()
    ...
    try:
        ...
        terminated_after_render = (
            screenshot.exists() and screenshot.stat().st_size >= 512
        )
    ...
    return {
        ...
        "fault": (
            None
            if rendered and completed_cleanly
            else ("timeout" if deadline_reached else "browser_render_failure")
        ),
        "physical_boundary_hit": bool(deadline_reached and not rendered),
    }


def verify_candidate(
    case: dict[str, Any], source: str, config: dict[str, Any]
) -> dict[str, Any]:
    started = time.monotonic()
    ...
    if kind == "blind_rubric_v1":
        return _result(
            case,
            passed=False,
            stage="blind_rubric_pending",
            started=started,
            pending_human_judgment=True,
        )
    return _result(
        case, passed=False, stage="dispatch", started=started, fault="unknown_verifier"
    )
...
def _prohibited_side_effect(arm_id: str, source: str) -> str:
    patterns = {
        "python": [
            ...
            r"\bimport\s+(?:os|socket|subprocess|pathlib|http|urllib|requests)\b",
            r"\bfrom\s+(?:os|socket|subprocess|pathlib|http|urllib|requests)\b",
        ],
        "javascript_typescript": [
            r"\b(?:fetch|XMLHttpRequest|WebSocket|eval)\s*\(?",
            r"\bDeno\s*\.",
            r"\b(?:require|import)\s*\(?[\"'](?:node:|fs|net|http|https|child_process)",
        ],
        "rust": [
...
def _verify_python(
    case: dict[str, Any], source: str, config: dict[str, Any], started: float
) -> dict[str, Any]:
    try:
        ast.parse(source)
...
"""
        ...
        if spec["family"] == "repository_edit":
            ...
            if invalid_args is not None:
                ...
                if (
                    not run["ok"]
                    or observed.get("state") != "raised"
                    or observed.get("exception_type") not in {"ValueError", "TypeError"}
                ):
                    passed = False
        fault = (
            None
            if passed
                ...
                (run.get("fault") for run in executions if run.get("fault")),
                "test_failure",
            )
        )
        return _result(
            case,
            passed=passed,
            stage="input_only_subprocess_tests",
            started=started,
...
"""
        ...
        if spec["family"] == "repository_edit":
            ...
            if invalid_args is not None:
                ...
                if (
                    not run["ok"]
                    or observed.get("state") != "raised"
                    or observed.get("exception_type")
                    not in {"Error", "RangeError", "TypeError"}
                ):
                    passed = False
        ...
        fault = (
            ...
            if passed
                ...
                (run.get("fault") for run in checks if run.get("fault")), "test_failure"
            )
        )
        return _result(
            case,
...
def _rust_value(value: Any) -> str:
    ...
    if isinstance(value, list):
        if value and isinstance(value[0], dict):
            return (
                "vec!["
                + ",".join(
                    f"({_rust_string(v['name'])},{_rust_value(v['enabled'])},{v['priority']})"
                    for v in value
                )
                + "]"
            )
    ...
    if isinstance(value, dict):
        return (
            "vec!["
            + ",".join(f"({_rust_string(k)},{v})" for k, v in sorted(value.items()))
            + "]"
        )
...
def _rust_assertion(spec: dict[str, Any], row: dict[str, Any]) -> str:
    ...
    if family in {"stable_unique", "normalize_slug", "select_active"}:
        call = (
            f"{name}(&{_rust_value(args[0])})"
            if family != "normalize_slug"
            else f"{name}({json.dumps(args[0])})"
        )
...
def _verify_rust(
    ...
    with tempfile.TemporaryDirectory(prefix="theseus-fu-") as raw:
        ...
        timeout = int(config["sandbox"]["rust_timeout_seconds"])
        checks = []
        for args in (
        ...
        return _result(
            case,
            passed=passed,
            stage="cargo_check_clippy_test",
            started=started,
            execution=checks,
            fault=None if passed else checks[-1].get("fault", "test_failure"),
        )
...
class _DOMAudit(html.parser.HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        self.attrs.append(
            (tag, {key: "" if value is None else value for key, value in attrs})
        )
...
def _verify_html(
    ...
    if re.search(r"(?:src|href)\s*=\s*['\"]file:", lower) or re.search(
        r"@import\b", lower
    ):
    ...
    for tag, key, value in spec["required_attrs"]:
        if not any(
            found_tag == tag and attrs.get(key) == value
            for found_tag, attrs in audit.attrs
        ):
            ...
            for width, height, label in ((800, 600, "wide"), (375, 667, "narrow")):
                ...
                render = _render_chrome(
                    [
                        ...
                        "--dump-dom",
                        f"--user-data-dir={workdir / f'chrome-{label}'}",
                        f"--window-size={width},{height}",
                        f"--screenshot={screenshot}",
                        (workdir / "audit.html").as_uri(),
                    ],
                    workdir,
                    screenshot,
                    int(config["sandbox"]["browser_render_timeout_seconds"]),
                )
                ...
                render["browser_assertions"] = _browser_assertions(
                    case["task_family"], audit, label
                )
                if render["ok"]:
                    render["screenshot_sha256"] = hashlib.sha256(
    ...
    passed = (
        not failures and len(renders) == 2 and all(render["ok"] for render in renders)
    )
    physical_timeout = any(render.get("fault") == "timeout" for render in renders)
    return _result(
        case,
        passed=passed,
        stage="parse_dom_a11y_responsive_render",
        started=started,
        failures=failures,
        tidy=tidy,
        renders=renders,
        fault=(
            ...
            if passed
                ...
                "timeout"
                if physical_timeout
                else (failures[0] if failures else "render_failure")
            )
        ),
    )
...
def _browser_assertions(
    family: str, audit: dict[str, Any], viewport: str
) -> dict[str, bool]:
    assertions = {
    ...
    if family == "accessible_form":
        assertions.update(
            ...
            inputs_labeled=int(audit.get("labeledInputCount") or 0)
            == int(audit.get("inputCount") or 0),
        ...
        assertions["table_headers_present"] = (
            int(audit.get("tableHeaderCount") or 0) >= 3
        )
        ...
        assertions["theme_variables_resolve"] = (
            int(audit.get("themeVariableCount") or 0) == 3
        )
    elif family == "modal_dialog":
        assertions["dialog_visible"] = audit.get("dialogVisible") is True
        assertions["dialog_actions_named"] = (
            int(audit.get("namedButtonCount") or 0) >= 2
        )
...
def score_english_judgments(
    ...
    for row in judgments:
        ...
        if set(scores) != set(dimensions) or any(
            not isinstance(scores[d], int) or not 0 <= scores[d] <= 4
            for d in dimensions
        ):
    ...
    for case_id, case in english.items():
        ...
        requires_adjudication = any(
            abs(first["scores"][d] - second["scores"][d]) >= delta for d in dimensions
        )
        ...
        final_scores = (
            adjudicated["scores"]
            if adjudicated
            else {d: (first["scores"][d] + second["scores"][d]) / 2 for d in dimensions}
        )
        pair_values.extend(
            (first["scores"][d], second["scores"][d]) for d in dimensions
        )
        for dimension in dimensions:
        ...
        concept_diagnostic = sum(term.lower() in text for term in required) / max(
            1, len(required)
        )
        ...
        results.append(
            {
                "case_id": case_id,
                "passed": mean >= 3.0 and not forbidden_hits,
                "mean_score": mean,
                "scores": final_scores,
                "required_concept_recall_diagnostic": concept_diagnostic,
...
def _quadratic_weighted_kappa(
    pairs: list[tuple[int, int]], categories: int = 5
) -> float | None:
    if not pairs:
        return None
