...
POLICY="project_theseus_vcm_sdist_batch_preflight_v1";STATE="PROSPECTIVE_TASK13_TRANSITIVE_SDIST_BATCH_STATIC_PREFLIGHT_V1";DEFAULT_CONFIG=ROOT/"configs/theseus_vcm_sdist_batch_preflight.json"
...
def execute(path:Path=DEFAULT_CONFIG)->dict[str,Any]:
 ...
 with tempfile.TemporaryDirectory(prefix="theseus-vcm-sdist-batch-",dir="/private/tmp") as raw:
  ...
  for row in p2a.dicts(cfg["packages"]):
   downloaded=temp/str(row["filename"]);cmd=["/usr/bin/curl","--fail","--location","--silent","--show-error","--max-time",str(limits["network_timeout_seconds"]),"--max-filesize",str(limits["maximum_single_download_bytes"]),"--output",str(downloaded),str(row["url"])];done=subprocess.run(cmd,stdout=subprocess.PIPE,stderr=subprocess.PIPE,check=False);payload=downloaded.read_bytes() if downloaded.is_file() else b"";row_faults=[]
   ...
   target=store/str(row["filename"])
   if not row_faults:
    store.mkdir(parents=True,exist_ok=True)
    if target.exists():row_faults.append("retained_sdist_already_exists")
    else:shutil.copyfile(downloaded,target)
   results.append({"name":row["name"],"version":row["version"],"download":{"command":cmd,"returncode":done.returncode,"bytes":len(payload),"sha256":hashlib.sha256(payload).hexdigest(),"stderr":done.stderr.decode("utf-8","replace"),"stderr_sha256":hashlib.sha256(done.stderr).hexdigest()},"inspection":inspection,"retained_sdist":{"path":p2a.rel(target),"bytes":target.stat().st_size,"sha256":p2a.sha256_file(target)} if target.is_file() else {},"faults":sorted(set(row_faults)),"disposition":"ELIGIBLE_FOR_NETWORK_DENIED_BATCH_BUILD" if eligible else "INCONCLUSIVE_IMPLEMENTATION_UNTRUSTED_BUILD_PREFLIGHT"})
