...
from neural_seed_token_decoder_support import PLAN_BODY_START_TOKEN, encode_target_rows, semantic_plan_from_body  # noqa: E402
...
from strict_generator_mlx_pretraining_probe import (  # noqa: E402
    ...
    SEMANTIC_SLOT_ROLES,
    semantic_body_action_operand_transition_aux_source_contrastive_weighted_loss_fn_mlx,
    semantic_body_action_operand_transition_aux_weighted_loss_fn_mlx,
    semantic_body_action_transition_aux_source_contrastive_weighted_loss_fn_mlx,
    semantic_body_action_transition_aux_weighted_loss_fn_mlx,
    body_transition_aux_weighted_loss_fn_mlx,
    body_transition_weight_rows,
    evaluate_body_action_mlx,
    evaluate_body_operand_mlx,
    evaluate_loss_mlx,
...
from strict_generator_mlx_adaptation_selection import (  # noqa: E402
    ...
    apply_semantic_construction_repair_profile,
    apply_guard_clean_target_weights,
    apply_source_condition_operation_coverage_sampling,
    build_semantic_ir_obligation_weighting_plan,
    config_with_teacher_curriculum_mode,
    load_adaptation_private_train_rows,
    private_train_balanced_sample_summary,
    private_train_tier_summary,
    private_train_tier_vocab_summary,
    reserve_governed_teacher_train_rows,
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "Honor configured curriculum adoption state, or explicitly enable/disable "
            "manifest-admitted teacher rows for matched causal ablations."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--body-action-loss-weight",
        type=float,
        default=0.0,
        help=(
            "Private prefix-conditioned body-action auxiliary loss weight. This trains a separate "
            "learned role head over broad body actions such as return, branch, block exit, update, "
            "and expression closure. It emits no code and grants no candidate-generation credit."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Private prefix-conditioned body-operand binding auxiliary loss weight. This trains a "
            "separate role head over visible parameters, loop variables, local state, builtin/method "
            "tokens, operators, literals, and delimiters. It emits no code and grants no "
    ...
    parser.add_argument(
        "--body-state-event-loss-weight",
        type=float,
        default=0.0,
        help=(
            "Train the dedicated prefix-conditioned state-machine event head. Labels are derived from "
            "private body action/operand roles and grant no generated-code credit."
        ),
    )
    ...
    config = config_with_teacher_curriculum_mode(
        read_json(resolve(args.config)),
        args.teacher_curriculum_mode,
    )
    ...
    report = run_adaptation(
        ...
        body_state_event_loss_weight=max(
            0.0,
            float(args.body_state_event_loss_weight if args.body_state_event_loss_weight is not None else 0.0),
        ),
        body_state_event_target_event_weight=max(
            0.0,
            float(
                args.body_state_event_target_event_weight
                if args.body_state_event_target_event_weight is not None
                else 1.0
            ),
        ),
        body_state_event_target_none_weight=max(
            0.0,
            float(
                args.body_state_event_target_none_weight
                if args.body_state_event_target_none_weight is not None
                else 0.20
            ),
        ),
...
def run_adaptation(
    ...
    *,
    ...
    train_body_action_targets, train_body_action_weights, body_action_train_summary = body_action_target_rows(
        train_target_rows,
    ...
    (
        ...
        train_body_state_event_summary,
    ) = body_state_event_target_rows(
        train_body_action_targets,
        train_body_operand_targets,
        train_body_action_weights,
        train_body_operand_weights,
        event_weight=body_state_event_target_event_weight,
        none_weight=body_state_event_target_none_weight,
    )
    (
        eval_body_state_event_targets,
        eval_body_state_event_weights,
        eval_body_state_event_summary,
    ) = body_state_event_target_rows(
        eval_body_action_targets,
        eval_body_operand_targets,
        eval_body_action_weights,
    ...
    body_action_requested = float(body_action_loss_weight or 0.0) > 0.0 and hasattr(model, "body_action_logits")
    body_action_enabled = (
        body_action_requested
        and int(body_action_train_summary.get("active_positions") or 0) > 0
        and int(body_action_eval_summary.get("active_positions") or 0) > 0
    )
    body_operand_requested = float(body_operand_loss_weight or 0.0) > 0.0 and hasattr(model, "body_operand_logits")
    body_operand_enabled = (
        body_operand_requested
        and body_action_enabled
        and body_transition_enabled
        and int(body_operand_train_summary.get("active_positions") or 0) > 0
    ...
    body_action_before = evaluate_body_action_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_action_targets,
        ...
        batch_size=batch_size,
        pad_id=pad_id,
        enabled=body_action_enabled,
        mx=mx,
        nn=nn,
    )
    body_operand_before = evaluate_body_operand_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_operand_targets,
        ...
        batch_size=batch_size,
        pad_id=pad_id,
        enabled=body_operand_enabled,
        mx=mx,
        nn=nn,
    )
    body_state_event_before = evaluate_body_state_event_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_state_event_targets,
    ...
    loss_and_grad_body_action_transition = (
        nn.value_and_grad(model, body_action_transition_aux_weighted_loss_fn_mlx)
        if body_action_enabled
        else None
    )
    loss_and_grad_body_action_operand_transition = (
        nn.value_and_grad(model, body_action_operand_transition_aux_weighted_loss_fn_mlx)
        if body_operand_enabled and body_action_enabled
        else None
    )
    loss_and_grad_body_action_operand_transition_event = (
        nn.value_and_grad(model, body_action_operand_transition_event_aux_weighted_loss_fn_mlx)
    ...
    loss_and_grad_semantic_body_action_transition = (
        nn.value_and_grad(model, semantic_body_action_transition_aux_weighted_loss_fn_mlx)
        if body_action_enabled and semantic_slot_enabled
        else None
    )
    loss_and_grad_semantic_body_action_operand_transition = (
        nn.value_and_grad(model, semantic_body_action_operand_transition_aux_weighted_loss_fn_mlx)
        if body_operand_enabled and body_action_enabled and semantic_slot_enabled
        else None
    )
    loss_and_grad_semantic_body_action_operand_transition_event = (
        nn.value_and_grad(model, semantic_body_action_operand_transition_event_aux_weighted_loss_fn_mlx)
    ...
    loss_and_grad_semantic_body_action_transition_contrast = (
        nn.value_and_grad(model, semantic_body_action_transition_aux_source_contrastive_weighted_loss_fn_mlx)
        if body_action_enabled and semantic_slot_enabled and source_contrastive_active
        else None
    )
    loss_and_grad_semantic_body_action_operand_transition_contrast = (
        nn.value_and_grad(model, semantic_body_action_operand_transition_aux_source_contrastive_weighted_loss_fn_mlx)
        if body_operand_enabled and body_action_enabled and semantic_slot_enabled and source_contrastive_active
        else None
    )
    ...
    body_transition_weight_matrix = (
        mx.array(train_body_transition_weights, dtype=mx.float32)
        if (body_transition_enabled or body_action_enabled)
        else None
    )
    body_action_target_matrix = mx.array(train_body_action_targets, dtype=mx.int32) if body_action_enabled else None
    body_action_weight_matrix = mx.array(train_body_action_weights, dtype=mx.float32) if body_action_enabled else None
    body_operand_target_matrix = mx.array(train_body_operand_targets, dtype=mx.int32) if body_operand_enabled else None
    body_operand_weight_matrix = mx.array(train_body_operand_weights, dtype=mx.float32) if body_operand_enabled else None
    body_state_event_target_matrix = (
        mx.array(train_body_state_event_targets, dtype=mx.int32)
        if body_state_event_enabled
        else None
    )
    body_state_event_weight_matrix = (
        mx.array(train_body_state_event_weights, dtype=mx.float32)
        if body_state_event_enabled
        else None
    )
    ...
    if body_action_enabled:
        mx.eval(body_action_target_matrix, body_action_weight_matrix)
    if body_operand_enabled:
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, len(order), batch_size):
            ...
            if body_action_enabled:
                body_action_targets = body_action_target_matrix[batch_indices]
                body_action_weights = body_action_weight_matrix[batch_indices]
            if body_operand_enabled:
                body_operand_targets = body_operand_target_matrix[batch_indices]
                body_operand_weights = body_operand_weight_matrix[batch_indices]
            if body_state_event_enabled:
            ...
            if pairwise_replay_active and loss_and_grad_pairwise_composite is not None:
                ...
                and body_transition_enabled
                and body_state_event_enabled
                and loss_and_grad_semantic_body_action_operand_transition_event_contrast is not None
            ):
                shifted_indices = indices[1:] + indices[:1]
                mismatched_src = source_matrix[mx.array(shifted_indices, dtype=mx.int32)]
                effective_contrastive_weight = source_contrastive_loss_weight if len(indices) > 1 else 0.0
                if semantic_plan_enabled:
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_operand_transition_event_contrast(
                    model,
                    src,
                    mismatched_src,
                    tgt,
                ...
                and body_action_enabled
                and body_transition_enabled
                and loss_and_grad_semantic_body_action_operand_transition_contrast is not None
            ):
                shifted_indices = indices[1:] + indices[:1]
                mismatched_src = source_matrix[mx.array(shifted_indices, dtype=mx.int32)]
                effective_contrastive_weight = source_contrastive_loss_weight if len(indices) > 1 else 0.0
                if semantic_plan_enabled:
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_operand_transition_contrast(
                    model,
                    src,
                    mismatched_src,
                    tgt,
                ...
                and body_action_enabled
                and body_transition_enabled
                and loss_and_grad_semantic_body_action_transition_contrast is not None
            ):
                shifted_indices = indices[1:] + indices[:1]
                mismatched_src = source_matrix[mx.array(shifted_indices, dtype=mx.int32)]
                effective_contrastive_weight = source_contrastive_loss_weight if len(indices) > 1 else 0.0
                if semantic_plan_enabled:
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_transition_contrast(
                    model,
                    src,
                    mismatched_src,
                    tgt,
                ...
                if semantic_plan_enabled:
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_operand_transition_event(
                    model,
                    src,
                    tgt,
                    pad_id,
                ...
                and body_action_enabled
                and body_transition_enabled
                and loss_and_grad_semantic_body_action_operand_transition is not None
            ):
                if semantic_plan_enabled:
                    plan_targets_arg = plan_targets
                    plan_weights_arg = plan_weights
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_operand_transition(
                    model,
                    src,
                    tgt,
                    pad_id,
                ...
                and body_action_enabled
                and body_transition_enabled
                and loss_and_grad_semantic_body_action_transition is not None
            ):
                if semantic_plan_enabled:
                    plan_targets_arg = plan_targets
                    plan_weights_arg = plan_weights
                    ...
                    plan_weights_arg = weights[:, 0]
                    effective_plan_weight = 0.0
                loss, grads = loss_and_grad_semantic_body_action_transition(
                    model,
                    src,
                    tgt,
                    pad_id,
            ...
            elif body_action_enabled and loss_and_grad_body_action_transition is not None:
                loss, grads = loss_and_grad_body_action_transition(
                    model,
                    src,
                    tgt,
                    pad_id,
    ...
    body_action_after = evaluate_body_action_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_action_targets,
    ...
    body_operand_after = evaluate_body_operand_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_operand_targets,
    ...
    body_state_event_after = evaluate_body_state_event_mlx(
        model,
        eval_source_rows,
        eval_target_rows,
        eval_body_state_event_targets,
    ...
    vocab_payload.update(
        {
            ...
            "body_action_auxiliary": {
                ...
                "weight": float(body_action_loss_weight or 0.0),
                "target_mode": target_mode,
                "train_targets": body_action_train_summary,
                "eval_targets": body_action_eval_summary,
                "roles": list(BODY_ACTION_ROLES),
            },
            "body_operand_auxiliary": {
                "enabled": bool(body_operand_enabled),
                "requested": bool(body_operand_requested),
                "weight": float(body_operand_loss_weight or 0.0),
                "target_mode": target_mode,
                "train_targets": body_operand_train_summary,
                "eval_targets": body_operand_eval_summary,
            ...
            "body_state_event_auxiliary": {
                ...
                "roles": list(BODY_STATE_EVENT_ROLES),
                "score_semantics": (
                    "Dedicated private state-machine event auxiliary head. Targets are derived from "
    ...
    payload = {
        ...
        "body_action_auxiliary": {
            "enabled": body_action_enabled,
            "requested": body_action_requested,
            "policy": "private_prefix_conditioned_body_action_auxiliary_v1" if body_action_enabled else "not_enabled",
            "weight": float(body_action_loss_weight or 0.0),
            "target_mode": target_mode,
            "train_targets": body_action_train_summary,
            "eval_targets": body_action_eval_summary,
            "heldout_action_loss_before": body_action_before.get("loss"),
            "heldout_action_loss_after": body_action_after.get("loss"),
            "heldout_action_accuracy_before": body_action_before.get("accuracy"),
            "heldout_action_accuracy_after": body_action_after.get("accuracy"),
            "heldout_action_improved": (
                body_action_enabled
                and body_action_before.get("loss") is not None
                and body_action_after.get("loss") is not None
                and float(body_action_after["loss"]) < float(body_action_before["loss"])
            ),
            "uses_eval_tests_or_solutions": False,
            "uses_public_data": False,
            "uses_answer_metadata": False,
            ...
            "candidate_generation_credit": 0,
            "score_semantics": (
                "Private prefix-conditioned body-action auxiliary supervision. It trains a learned "
                "structural role head over body-continuation positions, with roles such as return, "
                "branch, block exit, update operator, and expression closure. It emits no candidate "
                "by itself and grants no learned-generation credit without strict decode/verifier behavior."
            ),
        },
        "body_operand_auxiliary": {
            "enabled": body_operand_enabled,
            "requested": body_operand_requested,
            "policy": "private_prefix_conditioned_body_operand_auxiliary_v1" if body_operand_enabled else "not_enabled",
            "weight": float(body_operand_loss_weight or 0.0),
            "target_mode": target_mode,
            "train_targets": body_operand_train_summary,
            "eval_targets": body_operand_eval_summary,
            "roles": list(BODY_OPERAND_ROLES),
            "heldout_operand_loss_before": body_operand_before.get("loss"),
            "heldout_operand_loss_after": body_operand_after.get("loss"),
            "heldout_operand_accuracy_before": body_operand_before.get("accuracy"),
            "heldout_operand_accuracy_after": body_operand_after.get("accuracy"),
            "heldout_operand_improved": (
                body_operand_enabled
                and body_operand_before.get("loss") is not None
                and body_operand_after.get("loss") is not None
                and float(body_operand_after["loss"]) < float(body_operand_before["loss"])
            ),
            "uses_eval_tests_or_solutions": False,
            "uses_public_data": False,
            "uses_answer_metadata": False,
            ...
            "candidate_generation_credit": 0,
            "score_semantics": (
                "Private prefix-conditioned body-operand auxiliary supervision. It trains a learned "
                "binding/value role head over visible parameters, loop variables, local state, "
                "builtins, methods, operators, literals, and delimiters. It emits no candidate by "
                "itself and grants no learned-generation credit without strict decode/verifier behavior."
            ),
        },
        "body_state_event_auxiliary": {
            ...
            "eval_targets": eval_body_state_event_summary,
            "roles": list(BODY_STATE_EVENT_ROLES),
            "heldout_event_loss_before": body_state_event_before.get("loss"),
            "heldout_event_loss_after": body_state_event_after.get("loss"),
            "heldout_event_accuracy_before": body_state_event_before.get("accuracy"),
            "heldout_event_accuracy_after": body_state_event_after.get("accuracy"),
            "heldout_event_improved": (
                body_state_event_enabled
            ...
            "candidate_generation_credit": 0,
            "score_semantics": (
                "Dedicated prefix-conditioned state-machine event auxiliary supervision. Event labels "
                "are derived from admitted private body-action/body-operand roles and represent none, "
                "traversal/call, state update, control transition, return/finalizer, value expression, "
                "or statement boundary. It emits no candidate by itself and grants no learned-generation "
...
def semantic_construction_profile_missing_components(payload: dict[str, Any]) -> list[str]:
    ...
    if "body_action_auxiliary" in required and not bool(component("body_action_auxiliary").get("enabled")):
        missing.append("body_action_auxiliary")
    if "body_transition_auxiliary" in required and not bool(component("body_transition_auxiliary").get("enabled")):
        missing.append("body_transition_auxiliary")
    if "body_operand_auxiliary" in required and not bool(component("body_operand_auxiliary").get("enabled")):
        missing.append("body_operand_auxiliary")
    if "body_state_event_auxiliary" in required:
        item = component("body_state_event_auxiliary")
        train = dict_or_empty(item.get("train_targets"))
        if not bool(item.get("enabled")) or int(train.get("event_positions") or 0) <= 0:
            missing.append("body_state_event_auxiliary")
...
def build_gates(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        gate(
            "body_action_loss_improved_when_enabled",
            (
                not bool(dict_or_empty(payload.get("body_action_auxiliary")).get("enabled"))
                or bool(dict_or_empty(payload.get("body_action_auxiliary")).get("heldout_action_improved"))
            ),
            "soft",
            dict_or_empty(payload.get("body_action_auxiliary")),
        ),
        gate(
            "body_operand_loss_improved_when_enabled",
            (
                not bool(dict_or_empty(payload.get("body_operand_auxiliary")).get("enabled"))
                or bool(dict_or_empty(payload.get("body_operand_auxiliary")).get("heldout_operand_improved"))
            ),
            "soft",
            dict_or_empty(payload.get("body_operand_auxiliary")),
        ),
        gate(
            "body_state_event_loss_improved_when_enabled",
            (
