#!/usr/bin/env python3
"""Machine-authorize the sealed P4-v2r2-r1 recovery campaign."""
...
POLICY = "project_theseus_p4v2r2r1_autonomous_launch_v1"
DEFAULT_CONFIG = ROOT / "configs" / "theseus_p4v2r2r1_autonomous_launch.json"
BOUND_CAMPAIGN_COMMIT = "2e07a783c3a064e7a9cabf910cb04b35765c3c78"
...
def audit_bindings(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for path_key, hash_key in (
        ...
        owners.append({"owner": path_key, "path": p2a.rel(path), "expected_sha256": expected, "observed_sha256": observed, "passed": passed})
...
def preflight(config: dict[str, Any], *, config_path: Path, overrides: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if jobs is None:
        jobs = resource_owner.active_accelerator_jobs(p2a.strings(config.get("exclusive_accelerator_process_patterns")))
    ...
    return {
        ...
        "maximum_inference": "Authorizes exactly one local sealed recovery campaign under an exclusive lease; no D1, D2, training, serving, hosted inference, or book promotion.",
    }
...
def execute_once(config: dict[str, Any], *, config_path: Path) -> dict[str, Any]:
    ...
    lease = {"policy": POLICY, "lease_id": lease_id, "state": "RUNNING", "created_utc": p2a.now()}
    try:
    ...
    error = ""
    try:
        completed = subprocess.run([str(config["python"]), str(p2a.resolve(str(config["campaign"])))], cwd=ROOT, text=True, capture_output=True, check=False)
    except Exception as exc:  # noqa: BLE001
        error = f"{type(exc).__name__}: {exc}"[:2000]
    final = campaign.audit_campaign()
    complete = completed is not None and completed.returncode == 0 and final.get("trigger_state") == "GREEN" and final.get("complete_tasks") == 10
    lease.update({"state": "COMPLETED" if complete else "STOPPED_RETAIN_EVIDENCE", "completed_utc": p2a.now(), "child_returncode": completed.returncode if completed else None, "error": error})
...
def main() -> int:
    ...
    report = execute_once(config, config_path=config_path) if args.execute else preflight(config, config_path=config_path)
    p2a.write_json(p2a.resolve(str(config["report"])), report)
    campaign_audit = p2a.mapping(report.get("final_campaign_audit") or report.get("campaign_audit"))
    print(json.dumps({"trigger_state": report["trigger_state"], "launch_authorized": report["launch_authorized"], "complete_tasks": campaign_audit.get("complete_tasks"), "pending_tasks": campaign_audit.get("pending_tasks"), "failed_gates": report.get("failed_gates", []), "faults": report.get("faults", [])}, indent=2, sort_keys=True), flush=True)
