#!/usr/bin/env python3
"""Run the sealed P4-v2r2 recovery successor exactly once."""
...
POLICY = "project_theseus_p4v2r2r1_recovery_campaign_v1"
...
ROUTE_CANARY = ROOT / "reports" / "theseus_p4v2r2r1_route_canary_audit.json"
PROGRESS = ROOT / "reports" / "theseus_p4v2r2r1_campaign_progress.json"
POOL_SHA256 = "12fa1a25276bd2d3bbed497a4b6e2fef4ebd868c81d570ac7d9e3c2d5c65bfa5"
INSTRUMENT_SHA256 = "028b1ce44961df62ccd5076787bb61f5dd032faee5fc2431cf8566f8a8bdfb7e"
ROUTE_CANARY_SHA256 = "ee29d77e01041a307056e7fa7a08508b2ec3880572f74915f58879bfaedd05c9"
...
def result_paths(row: dict[str, Any]) -> dict[str, Path]:
    ...
    return {
        "run": ROOT / "reports" / f"theseus_p4v2r2r1_attempt1_{stem}_run.json",
        "evaluation": ROOT / "reports" / f"theseus_p4v2r2r1_attempt1_{stem}_evaluation.json",
    }
...
def audit_campaign(extra_faults: list[str] | None = None) -> dict[str, Any]:
    ...
    if pool.get("state") != "SEALED_BEFORE_SUCCESSOR_CANDIDATE_GENERATION":
        faults.append("pool_not_sealed")
    if pool.get("faults") != [] or int(pool.get("task_count") or 0) != 10:
        faults.append("pool_adequacy_invalid")
    if not all(int(pool.get(key) or 0) == 10 for key in (
        "green_evaluator_audits", "v2r2_oracle_replays_green", "dependency_corruptions_rejected"
    )):
    ...
    if instrument.get("generation_budget", {}).get("project_selected_quality_token_cap") is not None:
    ...
    for expected_index, row in enumerate(p2a.dicts(pool.get("tasks")), start=1):
        ...
        if run_exists:
            ...
            total_boundary_hits += sum(
                ...
                for row in custody["terminations"]
            )
...
def main() -> int:
    ...
    print(json.dumps({key: report[key] for key in (
        "trigger_state", "complete_tasks", "pending_tasks", "model_calls_retained",
        "physical_context_boundary_hits", "project_selected_quality_token_cap", "faults"
