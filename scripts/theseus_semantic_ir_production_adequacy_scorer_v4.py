#!/usr/bin/env python3
"""Blindly score the fresh statement-granular adequacy denominator."""
...
import theseus_semantic_ir_production_adequacy_fresh_v4_evaluator as fresh_evaluator
import theseus_semantic_ir_production_adequacy_runtime_v4 as runtime_v4


ROOT = Path(__file__).resolve().parents[1]
CONFIG = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_campaign_v4.json"
POOL = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_fresh_v4_task_pool.json"
POOL_SHA256 = "23533c38b0b57164bdd2f3e46bc493e128e2136d7fe855781672236fc779f0f6"
FRESH_QUALIFICATION = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_fresh_v4_evaluator.json"
FRESH_QUALIFICATION_SHA256 = "f3ae736ecae7be1c559181b29b862a90fac0ccd7dbfbba8daa216654b31a9662"
...
DEFAULT_RUN = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_candidates_v4.json"
DEFAULT_OUT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_score_v4.json"
POLICY = "project_theseus_semantic_ir_production_adequacy_score_v4"
...
def score(run_path: Path) -> dict[str, Any]:
    ...
    if (
        run.get("policy") != "project_theseus_semantic_ir_production_adequacy_candidates_v4"
    ...
    if run_config.get("path") != p2a.rel(CONFIG) or run_config.get("sha256") != p2a.sha256_file(CONFIG) or config.get("state") != "PROSPECTIVELY_BOUND_FRESH_V4_BEFORE_CANDIDATE_GENERATION":
    ...
    if pool.get("trigger_state") != "GREEN" or pool.get("state") != "SEALED_FRESH_STATEMENT_GRANULAR_DENOMINATOR_BEFORE_CANDIDATE_GENERATION":
...
def score_row(row: dict[str, Any], pool_row: dict[str, Any], source_row: dict[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"theseus-adequacy-score-v4-{index:02d}-") as directory:
    ...
    if not faults:
        try:
            passed = fresh_evaluator.evaluate(index, sources, tuple(selected)) if index <= 4 else base_evaluator.evaluate(index, sources, tuple(selected))
...
def evaluator_binding_faults() -> list[str]:
    ...
    if p2a.read_json(FRESH_QUALIFICATION).get("trigger_state") != "GREEN" or p2a.read_json(BASE_QUALIFICATION).get("trigger_state") != "GREEN":
