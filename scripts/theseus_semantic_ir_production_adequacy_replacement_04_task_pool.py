#!/usr/bin/env python3
"""Seal the candidate-visible packet for the independently qualified Task 4 replacement."""
...
EVALUATOR_REPORT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_replacement_04_evaluator.json"
EVALUATOR_REPORT_SHA256 = "aa1a4dce43d1a624dd4bab269732ec7c1b8240fdbf34f5303bbaf0a7315864b5"
TASK_PATH = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_replacement_04_task.json"
PACKET_PATH = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_replacement_04_candidate_packet.json"
DEFAULT_OUT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_replacement_04_task_pool.json"
POLICY = "project_theseus_semantic_ir_production_adequacy_replacement_04_task_pool_v1"
...
def build_packet(source_report: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any], str, int, list[str]]:
    ...
    selected = ["skbio/alignment/_pair.py"]
    task = {
        "policy": p4.TASK_POLICY,
        "state": "SEALED_BEFORE_CANDIDATE_GENERATION",
        "opaque_task_id": "semantic-ir-adequacy-04r1",
        "campaign_index": 4,
        "partition": "semantic_ir_production_implementation_adequacy",
        "family": "licensed_source_disjoint_python_causal_repair",
        "natural_request": (
            "Make pair_align treat an omitted absolute tolerance as zero before casting it to "
            "the alignment score dtype. Preserve explicit tolerance values, both alignment "
            "modes, and unrelated gap-cost behavior. Modify only skbio/alignment/_pair.py."
        ),
        ...
        "obligations": [
            {
                "id": "O1",
                "kind": "require",
                "text": "An omitted absolute tolerance is normalized to zero and cast through the alignment score dtype before tolerance-dependent use.",
            },
            {
                "id": "O2",
                "kind": "preserve",
                "text": "Explicit tolerance values, global and local alignment modes, gap-cost preparation, and unrelated alignment behavior remain intact.",
            },
            {
                "id": "O3",
                "kind": "non_goal",
    ...
    with tempfile.TemporaryDirectory(prefix="theseus-adequacy-04r1-packet-") as directory:
