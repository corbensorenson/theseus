#!/usr/bin/env python3
"""Fetch and offline-replay only task 36's exact Cargo dependency lock."""
...
POLICY="project_theseus_vcm_task36_dependency_canary_v1";STATE="PROSPECTIVE_TASK_36_EXACT_CARGO_DEPENDENCY_FETCH_AND_OFFLINE_REPLAY";DEFAULT_CONFIG=ROOT/"configs"/"theseus_vcm_task36_dependency_canary.json"
def main()->int:
...
def preflight(cfg:dict[str,Any],path:Path)->dict[str,Any]:
 ...
 schedule=p2a.dicts(reports.get("prefetch_plan",{}).get("schedule"));row=next((r for r in schedule if r.get("schedule_ordinal")==3),{});compat=next((r for r in p2a.dicts(reports.get("compatibility_v3",{}).get("rows")) if r.get("index")==36),{})
 if row.get("index")!=36 or row.get("manager")!="cargo" or p2a.mapping(row.get("governing_lock")).get("sha256")!="680461bbfb2b6e77bbf3387fc7999b0a1737cf340b311adc836db8a215940e42":faults.append("schedule_binding_invalid")
 if compat.get("state")!="COMPATIBLE_DECLARED_REQUIREMENTS" or "cargo1_97_1_rustc1_97_1" not in compat.get("compatible_profile_ids",[]):faults.append("compatibility_binding_invalid")
 if reports.get("rust_toolchain",{}).get("trigger_state")!="GREEN" or reports.get("task7_closure_audit",{}).get("trigger_state")!="GREEN":faults.append("predecessor_report_invalid")
 task=p2a.mapping(cfg.get("task"))
 if task.get("index")!=36 or task.get("repository")!="tomerwave/godlint" or task.get("expected_checksum_package_count")!=32:faults.append("task_binding_invalid")
 observed={}
 ...
 if len(checks)!=32 or identity!=task.get("expected_checksum_identity_sha256"):faults.append("lock_checksum_denominator_invalid")
 ...
 for k in ("minimum_free_bytes_after_execution","maximum_retained_bytes","maximum_single_written_file_mib","wall_seconds_per_command","cpu_seconds_per_command","output_mib","open_files","user_processes","max_process_group_rss_mib","terminate_grace_seconds"):
  if float(limits.get(k) or 0)<=0:faults.append(f"limit_invalid:{k}")
 store=p2a.resolve(str(cfg.get("retained_store") or ""))
 if store.parent!=(ROOT/"runtime/vcm_evaluator/dependency_store/cargo").resolve() or store.name!="task-36":faults.append("retained_store_invalid")
 return {"policy":POLICY,"created_utc":p2a.now(),"trigger_state":"RED" if faults else "PAUSED","state":"CONTRACT_INVALID" if faults else "READY_FOR_TASK_36_EXACT_CARGO_DEPENDENCY_CANARY","faults":sorted(set(faults)),"config":base.identity(path),"observed_parent_target_dependency_identities":observed,"lock_checksum_package_count":len(checks),"lock_checksum_identity_sha256":identity,"execution_performed":False,"network_enabled_dependency_fetches":0,"network_denied_offline_replays":0,"dependency_installations":0,"repository_build_executions":0,"repository_runner_executions":0,"parent_target_or_evaluator_executions":0,"candidate_or_control_calls":0,"external_reference_calls":0,"maximum_inference":cfg.get("maximum_inference")}
def execute(cfg:dict[str,Any],path:Path)->dict[str,Any]:
 ...
 with tempfile.TemporaryDirectory(prefix="theseus-vcm-task36-deps-",dir="/private/tmp") as raw:
  work=Path(raw).resolve();repo=work/"repository";cargo_home=work/"cargo-home";home=work/"home";tmp=work/"tmp";cargo_home.mkdir();home.mkdir();tmp.mkdir()
  target=p2a.mapping(p2a.mapping(cfg["archives"])["target"]);receipts["repository_extraction"],errs=base.safe_extract_repository(p2a.resolve(str(target["path"])),repo,str(target["archive_root"]));faults.extend(errs)
  source_before=base.tree_identity(repo,excluded_roots={"target"}) if not faults else {}
  ...
  if not faults:
   receipts["online_fetch"]=bounded.run_sandboxed(online,repo,work,env,cfg,network_denied=False)
   if base.command_failed(receipts["online_fetch"]):faults.append("online_dependency_fetch_failed")
  ...
  if not faults:
   receipts["offline_replay"]=bounded.run_sandboxed(offline,repo,work,env,cfg,network_denied=True)
   if base.command_failed(receipts["offline_replay"]):faults.append("offline_dependency_replay_failed")
  receipts["offline_cache"]=inspect_cache(cargo_home,cfg) if not faults else {}
  if not faults:faults.extend(f"offline:{e}" for e in receipts["offline_cache"]["faults"])
  source_after=base.tree_identity(repo,excluded_roots={"target"}) if repo.exists() else {};receipts["repository_source_before"]=source_before;receipts["repository_source_after"]=source_after
 ...
 receipts["retained_store"]=inspect_cache(store,cfg) if store.exists() else {};receipts["free_bytes_after"]=shutil.disk_usage(ROOT).free
...
def inspect_cache(home:Path,cfg:dict[str,Any])->dict[str,Any]:
 ...
 for row in expected:
  matches=list((home/"registry/cache").glob(f"*/{row['name']}-{row['version']}.crate")) if (home/"registry/cache").is_dir() else []
  good=[p for p in matches if hashlib.sha256(p.read_bytes()).hexdigest()==row["checksum"]]
...
def finish(before:dict[str,Any],faults:list[str],receipts:dict[str,Any],executed:bool)->dict[str,Any]:
 online=int("online_fetch" in receipts);offline=int("offline_replay" in receipts);return {**before,"created_utc":p2a.now(),"trigger_state":"GREEN" if executed and not faults else "RED","state":"TASK_36_EXACT_CARGO_DEPENDENCY_CACHE_ACQUIRED_AND_OFFLINE_REPLAY_QUALIFIED" if executed and not faults else "TASK_36_DEPENDENCY_CANARY_FAILED","faults":sorted(set(faults)),"execution_performed":executed,"network_enabled_dependency_fetches":online,"network_denied_offline_replays":offline,"receipts":receipts}
