#!/usr/bin/env python3
"""Independently score the sealed replacement-aware adequacy denominator."""
...
import theseus_semantic_ir_production_adequacy_replacement_02_evaluator as replacement_evaluator
...
DEFAULT_RUN = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_candidates_v2.json"
DEFAULT_OUT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_evaluation_v2.json"
POLICY = "project_theseus_semantic_ir_production_adequacy_evaluation_v2"
RUN_POLICY = "project_theseus_semantic_ir_production_adequacy_candidates_v2"
CONFIG = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_campaign_v2.json"
POOL = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_task_pool_v2.json"
...
REPLACEMENT_SOURCE = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_replacement_02_source.json"
REPLACEMENT_SOURCE_SHA256 = "b6a92bd9c8d73eae5f48f1ddf47eac8abce80a9b0dba926bcb18d3df27382680"
REPLACEMENT_QUALIFICATION = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_replacement_02_evaluator.json"
REPLACEMENT_QUALIFICATION_SHA256 = "dafacdca124b3849a808e21c0f9bf23ab57bb8180f28b3f5a8dbf77d7a10ee8e"
...
def score(run_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or int(run.get("preserved_candidate_count") or 0) != 1
        or int(run.get("new_candidate_count") or 0) != 17
    ...
    if (
        ...
        or config.get("state") != "PROSPECTIVELY_BOUND_AFTER_REPLACEMENT_POOL_SEAL_BEFORE_RESUME"
    ):
    ...
    if not faults:
        for row in run_rows:
            ...
            result = (
                score_replacement_row(row, source_rows[index])
                if index == 2
                else base_scorer.score_row(row, source_rows[index])
            )
    ...
    for result in results:
        receipt = by_stratum.setdefault(strata.get(int(result.get("index") or 0), ""), {"successes": 0, "tasks": 0})
        receipt["tasks"] += 1
        receipt["successes"] += int(result.get("passed") is True)
    stratum_floor = bool(by_stratum) and all(receipt["tasks"] == 3 and receipt["successes"] >= 2 for receipt in by_stratum.values())
    ...
    return {
        ...
        "maximum_inference": (
            ...
            "runtime, replacement-aware source panel, two-call repair regime, host-safety "
...
def evaluator_binding_faults() -> list[str]:
    ...
    for path, expected, label in (
        ...
        (REPLACEMENT_SOURCE, REPLACEMENT_SOURCE_SHA256, "replacement_source"),
    ...
    if p2a.read_json(BASE_QUALIFICATION).get("trigger_state") != "GREEN":
        faults.append("base_evaluator_qualification_invalid")
    if p2a.read_json(REPLACEMENT_QUALIFICATION).get("trigger_state") != "GREEN":
        faults.append("replacement_evaluator_qualification_invalid")
    return faults
...
def source_denominator() -> tuple[dict[int, dict[str, Any]], dict[int, str]]:
    ...
    replacement = p2a.read_json(REPLACEMENT_SOURCE)
    metadata = p2a.mapping(replacement.get("metadata"))
    rows[2] = {
        "index": 2,
        ...
        "selected_source_paths": ["business_logic_test.py"],
        "archives": replacement.get("archives"),
    }
    strata = {index: str(row.get("stratum") or "") for index, row in rows.items()}
    return rows, strata


def score_replacement_row(row: dict[str, Any], source_row: dict[str, Any]) -> dict[str, Any]:
    index = 2
    faults: list[str] = []
    ...
    with tempfile.TemporaryDirectory(prefix="theseus-adequacy-score-02r1-") as directory:
