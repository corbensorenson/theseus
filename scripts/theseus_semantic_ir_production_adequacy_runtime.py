...
CREATE_FILE_UNIT_RE = re.compile(
    r"UNIT ([A-Z][A-Z0-9_-]*)\n"
    r"OBLIGATIONS ([A-Z0-9_,]+)\n"
    r"OP CREATE_FILE\n"
    r"PATH ([^\n ]+)\n"
...
def grammar() -> str:
    return (
        ...
        "OP <REPLACE|INSERT_BEFORE|INSERT_AFTER|CREATE_FILE>\n"
        "PATH <exact repository-relative path>\n"
        "NODE <exact candidate-visible semantic node id; omit for CREATE_FILE>\n"
...
def complete(text: str) -> bool:
    ...
    return (
        ...
        and bool(UNIT_RE.search(raw) or CREATE_FILE_UNIT_RE.search(raw))
        and bool(
            re.search(
                r"^LOSS (?:NONE|[A-Z0-9_,]+)\nEND$",
                raw,
...
def semantic_scope_symbol_table(
    root: Path, task: dict[str, Any]
) -> dict[str, Any]:
    """Inventory existing semantic scopes and bind declared missing effect paths.

    Historical P4 symbol-table owners remain byte-stable.  The production owner
    adds only the missing-path state needed by CREATE_FILE, so older tasks retain
    their exact source digest and node identities.
    """
    ...
    if not missing:
        return p4s.semantic_scope_symbol_table(root, task)
    ...
    filtered["allowed_effect_paths"] = [path for path in allowed if path not in missing]
    table = p4s.semantic_scope_symbol_table(root, filtered)
    hashes = dict(p2a.mapping(table.get("allowed_file_sha256")))
    hashes.update({path: "MISSING_ALLOWED_EFFECT_PATH" for path in missing})
    return {
        **table,
        "source_digest": p2a.stable_hash({
            "allowed_file_sha256": hashes,
...
def parse(
    ...
    root: Path,
    *,
    symbol_table_factory: SymbolTableFactory | None = None,
) -> dict[str, Any]:
    ...
    try:
        symbols = (symbol_table_factory or semantic_scope_symbol_table)(root, task)
    except (OSError, p2a.InstrumentFault, ValueError):
        return empty(["semantic_symbol_table_invalid"], canonicalization)
    ...
    units = sorted(
        [
            *(("node", match) for match in UNIT_RE.finditer(raw)),
            *(("create", match) for match in CREATE_FILE_UNIT_RE.finditer(raw)),
        ],
    ...
    for unit_kind, match in units:
        if unit_kind == "create":
            unit_id, refs_raw, path_surface, replacement = match.groups()
        ...
        if unit_kind == "create":
            path_receipt = {
                ...
                "canonical": path_surface,
                "state": "EXACT_DECLARED_MISSING_PATH",
                "answer_bearing_transformation": False,
            }
            path_receipts.append(path_receipt)
            target = root / path_surface
            if (
                path_surface not in allowed
                or path_surface not in missing_allowed
                or p2a.unsafe_relative_path(path_surface)
                or target.exists()
                or path_surface in created_paths
                or not replacement.strip()
            ):
                faults.append("semantic_create_file_target_invalid")
                continue
            try:
                parent = target.parent.resolve(strict=True)
            except OSError:
                faults.append("semantic_create_file_parent_missing")
                continue
            root_resolved = root.resolve()
            if parent != root_resolved and root_resolved not in parent.parents:
                faults.append("semantic_create_file_target_invalid")
                continue
            created_paths.add(path_surface)
            actions.append({
                "op": CREATE_FILE_OPERATION,
                "path": path_surface,
            ...
            unit_receipts.append({
                "id": unit_id,
                "obligation_ids": refs,
                "change_obligation_ids": [
                    value for value in refs if value in change_ids
                ],
                "preserve_obligation_ids": [
                    value for value in refs if value in preserve_ids
                ],
                "operation": operation,
                "path": path_surface,
                "node_id": None,
...
def apply_actions(root: Path, actions: list[dict[str, Any]]) -> list[str]:
    """Apply production actions, adding files only after strict path validation."""

    faults: list[str] = []
    creates: dict[Path, str] = {}
    replacements: list[dict[str, Any]] = []
    root_resolved = root.resolve()
    for index, action in enumerate(actions):
        operation = str(action.get("op") or "REPLACE")
        if operation != CREATE_FILE_OPERATION:
            if operation != "REPLACE":
                faults.append(f"action_{index}_operation_unsupported")
            else:
                replacements.append(action)
            continue
        relative = str(action.get("path") or "")
        path = root / relative
        if p2a.unsafe_relative_path(relative):
            faults.append(f"action_{index}_create_path_unsafe")
            continue
        try:
            parent = path.parent.resolve(strict=True)
        except OSError:
            faults.append(f"action_{index}_create_parent_missing")
            continue
        if (
            (parent != root_resolved and root_resolved not in parent.parents)
            or path.exists()
            or path.is_symlink()
            or path in creates
        ):
            faults.append(f"action_{index}_create_target_unsafe_or_exists")
            continue
        replacement = str(action.get("replacement") or "")
        if not replacement.strip():
            faults.append(f"action_{index}_create_content_empty")
            continue
        creates[path] = replacement
    if faults:
        return faults
    faults.extend(p2a.apply_actions(root, replacements) if replacements else [])
    if faults:
        return faults
    for path, replacement in creates.items():
        try:
            path.write_text(
            ...
            faults.append(f"create_file_write_failed:{type(exc).__name__}")
    return faults
