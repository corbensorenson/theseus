#!/usr/bin/env python3
"""Run fresh v5 after replacing consumed v4 Task 1."""
...
DEFAULT_CONFIG = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_campaign_v5.json"
DEFAULT_OUT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_candidates_v5.json"
DEFAULT_JOURNAL = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_campaign_v5_journal.json"
POLICY = "project_theseus_semantic_ir_production_adequacy_candidates_v5"
CONFIG_POLICY = "project_theseus_semantic_ir_production_adequacy_campaign_v5"
AUDIT_POLICY = "project_theseus_semantic_ir_production_adequacy_campaign_audit_v5"
...
def audit_config(path: Path) -> dict[str, Any]:
    ...
    faults: list[str] = []
    if value.get("policy") != CONFIG_POLICY or value.get("state") != "PROSPECTIVELY_BOUND_FRESH_V5_BEFORE_CANDIDATE_GENERATION":
    ...
    if (
        ...
        or pool.get("state") != "SEALED_FRESH_V5_STATEMENT_GRANULAR_DENOMINATOR_BEFORE_CANDIDATE_GENERATION"
...
def run_campaign(config_path: Path, out_path: Path, journal_path: Path, *, session_factory: Callable[..., Any] = persistent_adequacy_session) -> dict[str, Any]:
    ...
    session = session_factory(
        ...
        required_revision=str(frozen.get("revision") or ""),
        required_snapshot_manifest_sha256=str(frozen.get("snapshot_manifest_sha256") or ""),
        session_id="semantic-ir-production-adequacy-panel-v5",
...
def run_task(pool_row: dict[str, Any], runtime_config: str, journal_path: Path, config_path: Path, completed_candidate_indices: list[int]) -> dict[str, Any]:
    ...
    def guarded_call(arm: str, task_id: str, call_number: int, call_prompt: str, maximum: int, runtime_config_value: str) -> dict[str, Any]:
        namespaced = f"semantic_ir_adequacy_fresh_v5_{index:02d}"
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"theseus-adequacy-v5-run-{index:02d}-") as directory:
            root = Path(directory) / "source"
            p2a.extract_source_archive(p2a.resolve(str(task.get("source_archive") or "")), root, str(task.get("source_archive_root") or ""))
            first = p2a.runtime_call(p2a.route_integrity.DIRECT_MODE, f"semantic_ir_adequacy_fresh_v5_{index:02d}", 1, prompt, MODEL_CONTEXT_TOKENS, runtime_config)
            ...
            second = p2a.runtime_call(p2a.route_integrity.DIRECT_MODE, f"semantic_ir_adequacy_fresh_v5_{index:02d}", 2, repair_prompt, MODEL_CONTEXT_TOKENS, runtime_config)
    ...
    telemetry = p4r.termination_telemetry([{"arm_id": f"semantic_ir_adequacy_fresh_v5_{index:02d}", "runtime_calls": [first["receipt"], second["receipt"]]}])
    infrastructure_faults: list[str] = []
...
def verify_candidate(task: dict[str, Any], actions: list[dict[str, Any]]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="theseus-adequacy-v5-verify-") as directory:
        root = Path(directory) / "source"
        p2a.extract_source_archive(p2a.resolve(str(task.get("source_archive") or "")), root, str(task.get("source_archive_root") or ""))
...
def write_journal(path: Path, config_path: Path, *, state: str, task_index: int, call_number: int, completed_candidate_indices: list[int], fault: str) -> None:
    p2a.write_json(path, {
        "policy": "project_theseus_semantic_ir_production_adequacy_campaign_journal_v5",
        "created_utc": p2a.now(),
        "state": state,
