#!/usr/bin/env python3
"""Run the fresh statement-granular 18-task adequacy denominator."""
...
DEFAULT_CONFIG = ROOT / "configs" / "theseus_semantic_ir_production_adequacy_campaign_v4.json"
DEFAULT_OUT = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_candidates_v4.json"
DEFAULT_JOURNAL = ROOT / "reports" / "theseus_semantic_ir_production_adequacy_campaign_v4_journal.json"
POLICY = "project_theseus_semantic_ir_production_adequacy_candidates_v4"
CONFIG_POLICY = "project_theseus_semantic_ir_production_adequacy_campaign_v4"
AUDIT_POLICY = "project_theseus_semantic_ir_production_adequacy_campaign_audit_v4"
...
def main() -> int:
    ...
    report = audit_config(config_path) if args.audit_only else run_campaign(config_path, p2a.resolve(args.out), p2a.resolve(args.journal))
...
def audit_config(path: Path) -> dict[str, Any]:
    ...
    faults: list[str] = []
    if value.get("policy") != CONFIG_POLICY or value.get("state") != "PROSPECTIVELY_BOUND_FRESH_V4_BEFORE_CANDIDATE_GENERATION":
    ...
    for path_key, digest_key in (
        ...
        ("statement_granular_runtime", "statement_granular_runtime_sha256"),
    ...
    if (
        ...
        or pool.get("state") != "SEALED_FRESH_STATEMENT_GRANULAR_DENOMINATOR_BEFORE_CANDIDATE_GENERATION"
    ...
    for key in ("external_inference_authorized", "teacher_calls_authorized", "training_rows_authorized", "serving_authorized", "D1_authorized", "D2_authorized", "book_support_promotion_authorized"):
    ...
    if contract.get("ready") is not True or identity.get("identity_sha256") != frozen.get("identity_sha256") or identity.get("decoder_sha256") != frozen.get("decoder_sha256"):
...
def persistent_adequacy_session(**kwargs: Any) -> Any:
    ...
    def generate_report(**request: Any) -> dict[str, Any]:
        ...
        if model is not None:
            report = backend_v2.attach_invalid_observation_diagnostic(report, model)
        return report
...
def run_campaign(config_path: Path, out_path: Path, journal_path: Path, *, session_factory: Callable[..., Any] = persistent_adequacy_session) -> dict[str, Any]:
    started = time.perf_counter()
    ...
    session = session_factory(
        ...
        required_revision=str(frozen.get("revision") or ""),
        required_snapshot_manifest_sha256=str(frozen.get("snapshot_manifest_sha256") or ""),
        session_id="semantic-ir-production-adequacy-panel-v4",
        completion_predicate=runtime_v4.complete,
    )
    if not session.ready:
        return campaign_report(config_path, audit, rows, ["persistent_backend_not_ready:" + ",".join(session.faults)], started, terminal=False)
    ...
    with base_campaign.assistant_runtime.bind_local_inference_runner(session.runtime_runner):
        for pool_row in pool_rows[len(rows):]:
            ...
            p2a.write_json(out_path, campaign_report(config_path, audit, rows, infrastructure_faults, started, terminal=False))
            write_journal(journal_path, config_path, state="TASK_SEALED" if not infrastructure_faults else "TASK_INFRASTRUCTURE_INVALID", task_index=index, call_number=2, completed_candidate_indices=[int(item.get("index") or 0) for item in rows], fault=infrastructure_faults[0] if infrastructure_faults else "")
...
def run_task(pool_row: dict[str, Any], runtime_config: str, journal_path: Path, config_path: Path, completed_candidate_indices: list[int]) -> dict[str, Any]:
    index = int(pool_row.get("index") or 0)
    ...
    def guarded_call(arm: str, task_id: str, call_number: int, call_prompt: str, maximum: int, runtime_config_value: str) -> dict[str, Any]:
        namespaced = f"semantic_ir_adequacy_fresh_v4_{index:02d}"
        ...
        result = original_runtime_call(arm, namespaced, call_number, call_prompt, maximum, runtime_config_value)
        telemetry = p4r.termination_telemetry([{"arm_id": namespaced, "runtime_calls": [result["receipt"]]}])
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"theseus-adequacy-v4-run-{index:02d}-") as directory:
            root = Path(directory) / "source"
            p2a.extract_source_archive(p2a.resolve(str(task.get("source_archive") or "")), root, str(task.get("source_archive_root") or ""))
            first = p2a.runtime_call(p2a.route_integrity.DIRECT_MODE, f"semantic_ir_adequacy_fresh_v4_{index:02d}", 1, prompt, MODEL_CONTEXT_TOKENS, runtime_config)
            first_candidate = runtime_v4.parse(first["assistant_text"], task, root)
            first_verification = verify_candidate(task, p2a.dicts(first_candidate.get("actions")))
            repair_prompt = runtime_v4.render_repair_prompt(prompt, first["assistant_text"], p2a.strings(first_candidate.get("faults")), first_verification)
            second = p2a.runtime_call(p2a.route_integrity.DIRECT_MODE, f"semantic_ir_adequacy_fresh_v4_{index:02d}", 2, repair_prompt, MODEL_CONTEXT_TOKENS, runtime_config)
            final_candidate = runtime_v4.parse(second["assistant_text"], task, root)
    ...
    calls = (first, second)
    route_receipts = [p2a.mapping(p2a.mapping(call.get("runtime_report")).get("route_integrity")) for call in calls]
    telemetry = p4r.termination_telemetry([{"arm_id": f"semantic_ir_adequacy_fresh_v4_{index:02d}", "runtime_calls": [first["receipt"], second["receipt"]]}])
    infrastructure_faults: list[str] = []
    if not all(receipt.get("ready") is True and receipt.get("release_allowed") is True for receipt in route_receipts):
    ...
    parse_apply_visible = bool(actions) and not final_candidate.get("faults") and not final_verification.get("apply_faults") and p2a.mapping(final_verification.get("visible_verifier")).get("passed") is True
    candidate_payload = {"actions": actions, "changed_paths": changed, "final_inventory_sha256": final_verification.get("final_inventory_sha256"), "semantic_receipt": final_candidate.get("semantic_receipt")}
    candidate_seal = {
    ...
    return {
        ...
        "candidate_mechanics_ready_for_hidden_evaluation": not infrastructure_faults and parse_apply_visible and authorized,
        ...
        "infrastructure_faults": sorted(set(infrastructure_faults)),
        "hidden_evaluator_executions": 0,
    }


def verify_candidate(task: dict[str, Any], actions: list[dict[str, Any]]) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="theseus-adequacy-v4-verify-") as directory:
        root = Path(directory) / "source"
        p2a.extract_source_archive(p2a.resolve(str(task.get("source_archive") or "")), root, str(task.get("source_archive_root") or ""))
...
def write_journal(path: Path, config_path: Path, *, state: str, task_index: int, call_number: int, completed_candidate_indices: list[int], fault: str) -> None:
    p2a.write_json(path, {
        "policy": "project_theseus_semantic_ir_production_adequacy_campaign_journal_v4",
        "created_utc": p2a.now(),
        "state": state,
