...
POLICY="project_theseus_vcm_replacement_resolution_audit_v3";DEFAULT_CONFIG=ROOT/"configs"/"theseus_vcm_replacement_resolution.json"
def main():
 p=argparse.ArgumentParser();p.add_argument("--config",default=p2a.rel(DEFAULT_CONFIG));p.add_argument("--out",default="");a=p.parse_args();path=p2a.resolve(a.config);cfg=p2a.read_json(path);r=audit(path);p2a.write_json(p2a.resolve(a.out or cfg["audit_report"]),r);print(json.dumps({k:r.get(k) for k in ("trigger_state","state","task_count","qualified_task_count","package_count","network_resolution_task_count","sealed_receipt_reuse_task_count","static_lock_task_count","parent_target_or_evaluator_executions","faults")},indent=2,sort_keys=True));return 0 if r["trigger_state"]=="GREEN" else 2
def audit(path=DEFAULT_CONFIG):
 ...
 rows=[];packages=network=static=reuse=0
 for row in p2a.dicts(producer.get("rows")):
  ...
  elif row.get("manager")=="receipt_reuse":reuse+=1
  else:network+=1
 ...
 if [r["index"] for r in rows]!=[12,13,35] or network!=1 or reuse!=1 or static!=1:faults.append("denominator_invalid")
 ...
 green=not faults
 return {"policy":POLICY,"audit_kind":"role-separated_rederivation","created_utc":p2a.now(),"trigger_state":"GREEN" if green else "RED","state":"THREE_REPLACEMENT_LOCKS_ROLE_SEPARATELY_REDERIVED" if green else "REPLACEMENT_RESOLUTION_AUDIT_FAILED","faults":sorted(set(faults)),"task_count":len(rows),"qualified_task_count":len(rows) if green else 0,"package_count":packages,"network_resolution_task_count":network,"sealed_receipt_reuse_task_count":reuse,"static_lock_task_count":static,"rows":rows,"package_installations":0,"source_build_executions":0,"repository_runner_executions":0,"parent_target_or_evaluator_executions":0,"local_model_calls":0,"external_reference_calls":0,"config":{"path":p2a.rel(path),"sha256":p2a.sha256_file(path)},"producer_report":{"path":p2a.rel(report_path),"sha256":p2a.sha256_file(report_path) if report_path.is_file() else ""},"maximum_inference":cfg.get("audit_maximum_inference")}
